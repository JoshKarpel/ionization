...
LOGMAN = si.utils.LogManager(
    "simulacra", "ionization", "idesolver", stdout_level=logging.DEBUG
)
...
def make_comparison_plot(identifier, rk4solvers, idesolvers):
    labels = [
        ...
        *[
            fr"IDE $\epsilon = {solver.global_error_tolerance}$ ({solver.elapsed_time.total_seconds()} s)"
            for solver in idesolvers
        ],
    ]
    line_kwargs = [
        *[{} for solver in rk4solvers],
...
if __name__ == "__main__":
    with LOGMAN as logger:
        for pw in [50, 100, 150, 200, 250, 300, 350, 400]:
            ...
            tb = 3
            dts = [5 * asec, 1 * asec, 0.5 * asec, 0.1 * asec]
            tols = [1e-4, 1e-5, 1e-6]
            maxiter = 400
            pts = 100
