...
if __name__ == "__main__":
    with si.utils.LogManager(
        ...
        logger.info(
            f"Time Points / Runtime (according to Sim): {round(time_points / sim.running_time.total_seconds(), 2)}"
        )
        logger.info(
            f"Time Points / Runtime (according to BlockTimer): {round(time_points / timer.proc_time_elapsed, 2)}"
        )
