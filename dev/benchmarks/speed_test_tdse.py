...
if __name__ == "__main__":
    with si.utils.LogManager(
        ...
        logger.info(
            f"Space-Time Points / Runtime (according to Sim): {round(space_time_points / sim.running_time.total_seconds())}"
        )
        logger.info(
            f"Space-Time Points / Runtime (according to BlockTimer): {round(space_time_points / timer.proc_time_elapsed.total_seconds())}"
        )
