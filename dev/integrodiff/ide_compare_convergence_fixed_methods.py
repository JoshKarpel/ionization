...
if __name__ == "__main__":
    with LOGMAN as logger:
        ...
        dts = np.geomspace(0.1, 20, 30) * asec
        ...
        results_by_method = {
            method: sorted(
                [r for r in results if r.spec.evolution_method == method],
                key=lambda x: x.spec.time_step,
            )
            for method in methods
        }

        b2_final_by_method = {
            method: np.array([r.b2[-1] for r in results])
            for method, results in results_by_method.items()
        }

        best_b2 = b2_final_by_method["RK4"][0]
        error_by_method = {
            method: np.abs(best_b2 - b2_final)
            for method, b2_final in b2_final_by_method.items()
        }
