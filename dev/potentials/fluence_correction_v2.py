...
if __name__ == '__main__':
    with LOGMAN as logger:
        ...
        for spec in specs:
            ...
            # print(sim.name)
            # print(sim.info())
            print(sim.name, sim.spec.electric_potential.get_fluence_numeric(sim.times) / u.Jcm2, sim.spec.electric_potential.get_electric_field_integral_numeric(sim.times) / (u.atomic_electric_field * u.atomic_time))
            # print()
        ...
        uncorrected = sim.spec.electric_potential.electric_potential.get_electric_field_amplitude(sim.times)
        print(corrected)
        print(uncorrected)
        print(corrected / uncorrected)
        print(np.sqrt(1.0 / .9997937024015667))
