...
if __name__ == '__main__':
    with si.utils.LogManager('simulacra', 'ionization', stdout_level = logging.DEBUG) as logger:
        ...
        ani = [
            src.ionization.animators.LineAnimator(postfix = '_zoom', length = 60, plot_limit = width * 20, **animator_kwargs),
            src.ionization.animators.LineAnimator(postfix = '_zoom_no_renorm', length = 60, plot_limit = width * 20, renormalize = False, **animator_kwargs),
            src.ionization.animators.LineAnimator(postfix = '_full', length = 60, **animator_kwargs),
            src.ionization.animators.LineAnimator(postfix = '_full_no_renorm', length = 60, renormalize = False, **animator_kwargs),
        ]
