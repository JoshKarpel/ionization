...
def run(spec):
    ...
    with si.utils.LogManager('simulacra', 'ionization', stdout_level = logging.DEBUG,
        ...
        si.vis.xy_plot(sim.name + '__energy_vs_time__ratio',
                         sim.times, np.abs(sim.spec.initial_state.energy / sim.energy_expectation_value_vs_time_internal),
                         x_label = '$t$', x_unit = 'asec', y_label = r'$\left| \frac{E(t)}{E_0} \right|$',
                         target_dir = OUT_DIR)

        si.vis.xy_plot(sim.name + '__energy_vs_time__ratio_log',
                         sim.times, np.abs(sim.spec.initial_state.energy / sim.energy_expectation_value_vs_time_internal),
...
if __name__ == '__main__':
    with si.utils.LogManager('simulacra', 'ionization', stdout_level = logging.DEBUG) as logger:
        ...
        si.vis.xy_plot(prefix + '__combined__energy_vs_time',
                         times, *energy_expectation_value,
                         line_labels = labels,
                         x_label = '$t$', x_unit = 'asec', y_label = r'$E(t)$', y_unit = 'eV',
                         target_dir = OUT_DIR)

        si.vis.xy_plot(prefix + '__combined__energy_vs_time__error',
                         times, *energy_expectation_value_error,
                         line_labels = labels,
                         x_label = '$t$', x_unit = 'asec', y_label = r'$E(t) - E_0$', y_unit = 'eV',
                         target_dir = OUT_DIR)

        si.vis.xy_plot(prefix + '__combined__energy_vs_time__ratio',
                         times, *energy_expectation_value_ratio,
                         line_labels = labels,
                         x_label = '$t$', x_unit = 'asec', y_label = r'$\left| \frac{E(t)}{E_0} \right|$',
                         target_dir = OUT_DIR)

        si.vis.xy_plot(prefix + '__combined__energy_vs_time__ratio_log',
                         times, *energy_expectation_value_ratio,
                         line_labels = labels,
                         x_label = '$t$', x_unit = 'asec', y_label = r'$\left| \frac{E(t)}{E_0} \right|$', y_log_axis = True,
                         target_dir = OUT_DIR)
