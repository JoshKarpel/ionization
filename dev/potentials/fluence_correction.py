...
if __name__ == "__main__":
    with LOGMAN as logger:
        ...
        cos_integral_of_field = dc_corrected_cos_pulse.get_electric_field_integral_numeric(
            times
        )
        sin_integral_of_field = dc_corrected_sin_pulse.get_electric_field_integral_numeric(
            times
        )
        print(
        ...
        corrected_cos_integral_of_field = fluence_and_dc_corrected_cos_pulse.get_electric_field_integral_numeric(
            times
        )
        corrected_sin_integral_of_field = fluence_and_dc_corrected_sin_pulse.get_electric_field_integral_numeric(
            times
        )
        print(
        ...
        print(
            f"sin correction ratio: {fluence_and_dc_corrected_sin_pulse.amplitude_correction_ratio}"
        )
        ...
        si.vis.xy_plot(
            ...
            line_labels=["cos", "sin", "cos corrected", "sin corrected"],
            line_kwargs=[
                {"linestyle": "-"},
                {"linestyle": "-"},
                {"linestyle": "--"},
        ...
        cos_freq, cos_fft = fft_field(
            dc_corrected_cos_pulse.get_electric_field_amplitude(times), times
        )
        sin_freq, sin_fft = fft_field(
            dc_corrected_sin_pulse.get_electric_field_amplitude(times), times
        )
        cos_power = np.abs(cos_fft) ** 2
        sin_power = np.abs(sin_fft) ** 2

        corrected_cos_freq, corrected_cos_fft = fft_field(
            fluence_and_dc_corrected_cos_pulse.get_electric_field_amplitude(times),
            times,
        )
        corrected_sin_freq, corrected_sin_fft = fft_field(
            fluence_and_dc_corrected_sin_pulse.get_electric_field_amplitude(times),
            times,
        )
        ...
        si.vis.xxyy_plot(
            "power_vs_freq",
            [cos_freq, sin_freq, corrected_cos_freq, corrected_sin_freq],
            [cos_power, sin_power, corrected_cos_power, corrected_sin_power],
            line_labels=["cos", "sin", "cos corrected", "sin corrected"],
            line_kwargs=[
                {"linestyle": "-"},
                {"linestyle": "-"},
