...
ANIM_KWARGS = dict(length=30, target_dir=OUT_DIR)
...
if __name__ == "__main__":
    with si.utils.LogManager(
        ...
        mass = electron_mass
        truncated_qho_pot = ion.HarmonicOscillator.from_energy_spacing_and_mass(
            energy_spacing=10 * eV, mass=mass, cutoff_distance=2 * bohr_radius
        )
        ...
        animators = [
            animation.animators.RectangleSplitLowerAnimator(
                ...
                axman_lower_right=animation.animators.WavefunctionStackplotAxis(
                    states=(
                        ion.QHOState.from_potential(
                            truncated_qho_pot, mass=electron_mass, n=n
                        )
                        for n in range(4)
                    ),
                    show_norm=False,
        ...
        sim.plot_state_overlaps_vs_time(
            time_unit="fsec", show_vector_potential=False, **PLOT_KWARGS
        )
