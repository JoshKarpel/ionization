...
if __name__ == "__main__":
    with si.utils.LogManager(
        ...
        for time_index, snapshot in sim.snapshots.items():
            for log in (True, False):
                theta, wavenumber, ip = snapshot.data[
                    "inner_product_with_plane_waves__free_only"
                ]

                sim.mesh.plot_electron_momentum_spectrum_from_meshes(
                    theta,
                    wavenumber,
                    ip,
                    "wavenumber",
                ...
                si.vis.xy_plot(
                    ...
                    np.abs(ip[0, :]) ** 2,
                    x_unit="per_nm",
                    x_label=r"Wavenumber $wavenumber$",
                    y_log_axis=log,
                    **snapshot_spectrum_kwargs,
                )
