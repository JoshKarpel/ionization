        ...
        with open(os.path.join(OUT_DIR, "overlaps.txt"), mode="w") as f:
            with contextlib.redirect_stdout(f):
                ...
                for analytic_state in test_states:
                    ...
                    analytic_mesh /= np.sqrt(
                        sim.mesh.inner_product_multiplier
                        * np.sum(np.abs(analytic_mesh) ** 2)
                    )
                    ...
                    for numeric_state, numeric_mesh in numerical_basis.items():
                        ...
                        overlaps[numeric_state] = (
                            np.abs(
                                np.sum(conj_analytic_mesh * numeric_mesh)
                                * sim.mesh.inner_product_multiplier
                            )
                            ** 2
                            if analytic_state.l == numeric_state.l
                            else 0
                        )
                    ...
                    for numeric_state, overlap in sorted(
                        ...
                        if overlap != 0:
                            print(
                                "<A|N>  |{}{}|^2 = {}".format(
                                    analytic_state.bra, numeric_state.ket, overlap
                                )
                            )
