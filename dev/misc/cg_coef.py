...
def triple_y_integral(j1, m1, j2, m2, j, m):
    ...
    result = si.math.complex_nquad(
        integrand, [(0, pi), (0, twopi)], opts={"limit": 1000}
    )
    logger.debug(result)
...
if __name__ == "__main__":
    with logman as logger:
        print(cg_coef(0.5, 0.5, 0.5, 0.5, 1, 1), 1)

        print(cg_coef(1, 1, 0.5, 0.5, 1.5, 1.5), 1)

        print(cg_coef(1, 1, 0.5, -0.5, 1.5, 0.5), np.sqrt(1 / 3))
        ...
        print(
            integ.dblquad(lambda x, y: x * y, 0, 0.5, lambda y: 0, lambda y: 1 - 2 * y)
        )
        print(
            si.math.complex_dblquad(
                lambda x, y: x * y, 0, 0.5, lambda y: 0, lambda y: 1 - 2 * y
            )
        )
        ...
        print(
            "num",
            triple_y_integral(30, 18, 21, -5, 19, 13),
            0.5
            * np.sqrt(2623 / (39 * pi))
            * cg_coef(30, 0, 21, 0, 19, 0)
            * cg_coef(30, 18, 21, -5, 19, 13),
        )
        print(
            "num",
            triple_y_integral(30, 0, 21, 0, 19, 0),
            0.5
            * np.sqrt(2623 / (39 * pi))
            * cg_coef(30, 0, 21, 0, 19, 0)
            * cg_coef(30, 0, 21, 0, 19, 0),
        ...
        print(
            "exact",
            pre(args[0], args[2], args[4])
            * cg_coef(args[0], 0, args[2], 0, args[4], 0)
            * cg_coef(*args),
        )
