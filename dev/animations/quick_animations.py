...
if __name__ == "__main__":
    with si.utils.LogManager(
        ...
        initial_state = ion.HydrogenBoundState(1, 0, 0) + ion.HydrogenBoundState(
            2, 1, 0
        )
        ...
        e_field = ion.SineWave.from_frequency(
            1 / (50 * asec), amplitude=1 * atomic_electric_field, window=window
        )
        mask = ion.RadialCosineMask(
            inner_radius=(bound - 25) * bohr_radius, outer_radius=bound * bohr_radius
        )
        ...
        animators = [
            ...
            src.ionization.animators.CylindricalSliceAnimator(
                postfix="_br", target_dir=OUT_DIR, distance_unit="bohr_radius"
            ),
        ]
        specs.append(
            ion.CylindricalSliceSpecification(
                ...
                mask=ion.RadialCosineMask(
                    inner_radius=15 * bohr_radius, outer_radius=20 * bohr_radius
                ),
                animators=animators,
            )
        )
        ...
        animators = [
            src.ionization.animators.SphericalSliceAnimator(
                postfix="_nm", target_dir=OUT_DIR, distance_unit="nm"
            ),
            src.ionization.animators.SphericalSliceAnimator(
                postfix="_br", target_dir=OUT_DIR, distance_unit="bohr_radius"
        ...
        init = ion.QHOState.from_potential(
            pot, mass, n=1
        ) + ion.QHOState.from_potential(pot, mass, n=2)
        ...
        specs.append(
            ion.LineSpecification(
                ...
                test_states=(
                    ion.QHOState.from_potential(pot, mass, n=n) for n in range(20)
                ),
                initial_state=init,
                time_initial=t_init * asec,
