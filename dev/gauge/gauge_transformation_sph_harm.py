...
def wrapped_plot_g_1d(sim):
    if (
        sim.time_index % (sim.time_steps // 20) == 0
        or sim.time_index == sim.time_steps - 1
        or sim.time_index < 20
    ):
        sim.mesh.plot_g_repr(name_postfix=f"_{sim.time_index}", **PLOT_KWARGS)
        sim.mesh.plot_g_repr(
            name_postfix=f"__TEST__{sim.time_index}",
            norm=si.vis.RichardsonNormalization(),
            **PLOT_KWARGS,
        )
...
if __name__ == "__main__":
    with logman as logger:
        photon_energy = 1 * eV
        amp = 0.01 * atomic_electric_field
        t_bound = 2
