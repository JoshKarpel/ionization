...
def run_spec(spec):
    with log as logger:
        try:
            sim = ion.ElectricFieldSimulation.load(
                os.path.join(LIB_DIR, spec.name + ".sim")
            )
...
def plot_final_initial_state_vs_r_points(name, sims, ref, **kwargs):
    ...
    final_initial_overlaps = [
        np.abs(
            sim.state_overlaps_vs_time[sim.spec.initial_state][-1]
            - ref.state_overlaps_vs_time[sim.spec.initial_state][-1]
        )
        for sim in sims
    ][:-1]
