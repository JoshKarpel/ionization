...
if __name__ == '__main__':
    ...
    with si.utils.LogManager('simulacra', 'ionization', stdout_level = 31 - ((args.verbosity + 1) * 10)) as logger:
        ...
        number_of_cycles = clu.Parameter(name = 'number_of_cycles',
                                         value = np.array(clu.ask_for_eval('Number of Cycles?',
                                                                           default = '[2, 3, 4]')),
                                         expandable = True)
        pulse_parameters.append(number_of_cycles)

        pulses = tuple(pulse_type.from_number_of_cycles(**d,
        ...
        for ii, spec_kwargs in tqdm(enumerate(spec_kwargs_list)):
            ...
            spec.number_of_cycles = electric_potential.number_of_cycles
            spec.number_of_pulse_widths = electric_potential.number_of_pulse_widths
