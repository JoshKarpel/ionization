...
if __name__ == '__main__':
    ...
    with si.utils.LogManager('simulacra', 'ionization', stdout_level = 31 - ((args.verbosity + 1) * 10)) as logger:
        # job type options
        job_processor = iclu.ElectricFieldJobProcessor
        ...
        if spec_type == ion.SphericalHarmonicSpecification:
            ...
            if numeric_basis_q:
                ...
                parameters.append(clu.Parameter(name = 'numeric_eigenstate_max_angular_momentum',
                                                value = clu.ask_for_input('Numeric Eigenstate Maximum l?', default = 20, cast_to = int)))
                parameters.append(clu.Parameter(name = 'numeric_eigenstate_max_energy',
                                                value = eV * clu.ask_for_input('Numeric Eigenstate Max Energy (in eV)?', default = 20, cast_to = float)))
        ...
        # internal potential
        if clu.ask_for_bool('Use softened Coulomb potential?', default = 'y'):
            parameters.append(clu.Parameter(name = 'internal_potential',
                                            value = ion.SoftCoulomb(softening_distance = bohr_radius * clu.ask_for_input('Softening Distance (in Bohr radii)?',
                                                                                                                         default = .05, cast_to = float))))
        ...
        photon_energy = clu.Parameter(name = 'photon_energy',
                                      value = eV * np.array(clu.ask_for_eval('Photon Energies (in eV)?', default = '[.5, 1, 1.5, 2, 4]')),
                                      expandable = True)
        pulse_parameters.append(photon_energy)

        intensity = clu.Parameter(name = 'intensity',
                                  value = (TW / (cm ** 2)) * np.array(clu.ask_for_eval('Pulse Intensity (in TW/cm^2)', default = 'np.logspace(1, 3, 20)')),
                                  expandable = True)
        pulse_parameters.append(intensity)
        ...
        phases = clu.Parameter(name = 'phase',
                               value = np.array(clu.ask_for_eval('Pulse CEP (in rad)?', default = '[pi / 2]')),
                               expandable = True)
