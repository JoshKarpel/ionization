...
if __name__ == '__main__':
    ...
    with si.utils.LogManager('simulacra', 'ionization', stdout_level = 31 - ((args.verbosity + 1) * 10)) as logger:
        ...
        amplitude_prefactor = clu.Parameter(name = 'amplitude_prefactor',
                                            value = atomic_electric_field * np.array(clu.ask_for_eval('Pulse Amplitude Prefactor (in atomic electric fields)?', default = 'np.linspace(0, 3, 11)[1:]')),
                                            expandable = True)
        pulse_parameters.append(amplitude_prefactor)
