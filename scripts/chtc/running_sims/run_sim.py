...
def parse_args():
    ...
    parser.add_argument("sim_name", type=str, help="the filename of the sim to run")
...
def main():
    ...
    with logman as logger:
        try:
            logger.info(
                f"Landed on execute node {socket.getfqdn()} ({socket.gethostbyname(socket.gethostname())}) at {datetime.datetime.utcnow()}."
            )
