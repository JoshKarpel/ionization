...
class MeshSimulationPlotter:
    ...
    def attach_electric_potential_plot_to_axis(
        ...
        if show_vector_potential:
            axis.plot(
                self.sim.data_times / time_unit_value,
                u.proton_charge * self.sim.data.vector_potential_amplitude / u.atomic_momentum,
    ...
    def wavefunction_vs_time(
        ...
        with si.vis.FigureManager(name = getattr(self, plot_name_from) + '__wavefunction_vs_time', **kwargs) as figman:
            ...
            state_overlaps = self.sim.data.state_overlaps  # it's a property that would otherwise get evaluated every time we asked for it

            extra_bound_overlap = np.zeros(self.sim.data_time_steps)
            if collapse_bound_state_angular_momenta:
                overlaps_by_n = {n: np.zeros(self.sim.data_time_steps) for n in range(1, bound_state_max_n + 1)}  # prepare arrays to sum over angular momenta in, one for each n
    ...
    def energy_spectrum(
        ...
        with si.vis.FigureManager(self.sim.name + '__energy_spectrum', **kwargs) as figman:
            fig = figman.fig
            ...
            ax.set_title(rf'Wavefunction Overlap by Energy at $ t = {u.uround(self.sim.times[time_index], time_unit)} \, {time_unit_str} $')
    ...
    def radial_position_expectation_value_vs_time(self, use_name: bool = False, **kwargs):
        if not use_name:
    ...
    def dipole_moment_vs_frequency(
        ...
        frequency, dipole_moment = self.sim.dipole_moment_vs_frequency(gauge = gauge, first_time = first_time, last_time = last_time)
...
class SphericalHarmonicSimulationPlotter(MeshSimulationPlotter):
    ...
    def angular_momentum_vs_time(self, use_name: bool = False, log: bool = False, renormalize: bool = False, **kwargs):
        ...
        if not isinstance(self.spec.electric_potential, potentials.NoPotentialEnergy):
            ax_field.plot(self.sim.times / u.asec, self.sim.data.electric_field_amplitude / u.atomic_electric_field, color = 'black', linewidth = 2)
