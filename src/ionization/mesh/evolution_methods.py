...
class MeshOperator(abc.ABC):
    def __init__(self, operator: meshes.SparseMatrixOperator, *, wrapping_direction: str):
...
class SimilarityOperator(DotOperator):
    def __init__(self, operator: meshes.SparseMatrixOperator, *, wrapping_direction: str, parity: str):
...
class SphericalHarmonicSplitOperator(EvolutionMethod):
    def evolve(self, mesh: 'meshes.SphericalHarmonicMesh', g: meshes.GMesh, time_step: complex) -> meshes.GMesh:
        ...
        split_operators = mesh._make_split_operator_evolution_operators(self.get_interaction_hamiltonian_matrix_operators(), tau)
