...
from .. import vis, core, exceptions
from . import meshes
...
def fmt_polar_axis(fig, axis, colormap, grid_kwargs, unit_latex):
    axis.grid(True, color = si.vis.CMAP_TO_OPPOSITE[colormap], **grid_kwargs)  # change grid color to make it show up against the colormesh
    ...
    axis.tick_params(axis = 'both', which = 'major', labelsize = 6)  # increase size of tick labels
    axis.tick_params(axis = 'y', which = 'major', colors = si.vis.CMAP_TO_OPPOSITE[colormap], pad = 3)  # make r ticks a color that shows up against the colormesh
