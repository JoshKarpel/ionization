...
class CylindricalSliceAnimator(WavefunctionSimulationAnimator):
    def _initialize_figure(self):
        ...
        self.ax_metrics = self.fig.add_axes([.065, .065, .87, .2])
        self.axman_metrics.assign_axis(self.ax_metrics)

        self.axis_managers += [self.axman_wavefunction, self.axman_metrics]
...
class PhiSliceAnimator(WavefunctionSimulationAnimator):
    ...
    def _initialize_figure(self):
        ...
        legend_kwargs = {'bbox_to_anchor': (1., 1.1),
                         ...
                         'fancybox': True,
                         'framealpha': .1}
        self.axman_metrics.legend_kwargs.update(legend_kwargs)
        self.ax_metrics = self.fig.add_axes([.575, .075, .36, .15])
        self.axman_metrics.assign_axis(self.ax_metrics)
