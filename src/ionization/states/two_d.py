...
class TwoDQuantumHarmonicOscillator(state.QuantumState):
    ...
    def __init__(
        ...
        if spring_constant_z <= 0:
            raise exceptions.InvalidQuantumStateParameter('spring_constant_z must be positive')
        if spring_constant_x <= 0:
            raise exceptions.InvalidQuantumStateParameter('spring_constant_x must be positive')
    ...
    def eval_z_x(self, z, x):
        kz = self.ksi_z * z
        kx = self.ksi_x * x
        norm = np.sqrt(self.ksi_z * self.ksi_x) / np.sqrt((2 ** (self.n_z + self.n_x)) * spmisc.factorial(self.n_z) * spmisc.factorial(self.n_x) * u.pi)
...
class TwoDGaussianWavepacket(state.QuantumState):
    ...
    def eval_z_x(self, z, x):
        ...
        gaussian = np.exp(-.5 * ((centered_z / self.width_z) ** 2)) * np.exp(-.5 * ((centered_x / self.width_x) ** 2))
    ...
    def __repr__(self):
        return utils.fmt_fields(self, 'center_z', 'center_x', 'width_z', 'width_x', 'k_z', 'k_x')
    ...
    @property
    def tex_ket(self):
        return fr'{state.fmt_amplitude_for_tex(self.amplitude)}\left| \phi \right\rangle'

    def info(self) -> si.Info:
        info = super().info()

        info.add_field('Mass', utils.fmt_quantity(self.mass, utils.MASS_UNITS))
        info.add_field('Z Center', utils.fmt_quantity(self.center_z, utils.LENGTH_UNITS))
        info.add_field('X Center', utils.fmt_quantity(self.center_x, utils.LENGTH_UNITS))
        ...
        info.add_field('Z Wavenumber', utils.fmt_quantity(self.k_z, utils.INVERSE_LENGTH_UNITS))
        info.add_field('X Wavenumber', utils.fmt_quantity(self.k_x, utils.INVERSE_LENGTH_UNITS))
