...
@hyp.given(
    fluence = st.floats(min_value = .001 * u.Jcm2, max_value = 10 * u.Jcm2),
    pulse_width = st.floats(min_value = 10 * u.asec, max_value = 1000 * u.asec),
    phase = st.floats(min_value = 0, max_value = u.twopi)
)
def test_sinc_pulse_fluence_is_exactly_target_fluence_after_correction(fluence, pulse_width, phase):
    pulse = ion.potentials.SincPulse(
        pulse_width = pulse_width,
        fluence = fluence,
        phase = phase,
        window = ion.potentials.SymmetricExponentialTimeWindow(
            window_time = 30 * pulse_width,
            window_width = .2 * pulse_width,
        )
    )
...
def test_sinc_pulse_fluence_is_exactly_target_fluence_after_correction_and_dc_correction(fluence, pulse_width, phase):
    pulse = ion.potentials.SincPulse(
        pulse_width = pulse_width,
        fluence = fluence,
        phase = phase,
