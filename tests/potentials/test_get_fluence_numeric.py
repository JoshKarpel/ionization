...
@hyp.given(
    amplitude = st.floats(min_value = -1 * u.atomic_electric_field, max_value = 1 * u.atomic_electric_field),
...
def test_fluence_for_rectangle(amplitude, duration):
    ...
    times = np.linspace(-.1 * duration, duration * 1.1, 100000)
...
@hyp.given(
    amplitude_1 = st.floats(min_value = -1 * u.atomic_electric_field, max_value = 1 * u.atomic_electric_field),
    amplitude_2 = st.floats(min_value = -1 * u.atomic_electric_field, max_value = 1 * u.atomic_electric_field),
...
def test_fluence_for_disjoint_rectangles(amplitude_1, amplitude_2, duration_1, delay, duration_2):
    ...
    times = np.linspace(-.1 * total_duration, 1.1 * total_duration, 1000000)
...
def test_fluence_for_overlapping_rectangles(amplitude_1, amplitude_2, duration_1, delay, duration_2):
    pot = ion.potentials.Rectangle(
        start_time = 0,
        end_time = duration_1,
        amplitude = amplitude_1,
    ...
    pot += ion.potentials.Rectangle(
        start_time = delay,
        end_time = delay + duration_2,
        amplitude = amplitude_2,
    )

    times = np.linspace(-.1 * duration_1, 1.1 * duration_1, 1000000)

    analytic_fluence = u.epsilon_0 * u.c * (amplitude_1 ** 2) * delay  # first rectangle is on
    analytic_fluence += u.epsilon_0 * u.c * ((amplitude_1 + amplitude_2) ** 2) * duration_2  # both on
    analytic_fluence += u.epsilon_0 * u.c * (amplitude_1 ** 2) * (duration_1 - duration_2 - delay)  # only first on
