...
class TestNoElectricPotential:
    ...
    def test_vector_potential_amplitude_is_always_zero(self, t):
        assert ion.potentials.NoElectricPotential().get_vector_potential_amplitude(t) == 0
...
class TestSineWave:
    ...
    def test_from_photon_energy_and_intensity(self):
        ...
        pot = ion.potentials.SineWave.from_photon_energy_and_intensity(photon_energy = photon_energy, intensity = intensity)

        assert np.allclose(pot.photon_energy, photon_energy)
        assert np.allclose(pot.intensity, intensity)

    @pytest.mark.parametrize('intensity', [
        1 * u.atomic_intensity,
        0,
    ])
    def test_can_construct_with_non_negative_intensity(self, intensity):
        ion.potentials.SineWave.from_photon_energy_and_intensity(photon_energy = 1 * u.eV, intensity = intensity)

    def test_cannot_construct_with_negative_intensity(self):
        with pytest.raises(ion.exceptions.InvalidPotentialParameter):
            ion.potentials.SineWave.from_photon_energy_and_intensity(photon_energy = 1 * u.eV, intensity = -1 * u.atomic_intensity)
...
@pytest.mark.parametrize('pulse_width', [
    0,
    -100 * u.asec
])
...
@pytest.mark.parametrize('fluence', [
    1 * u.Jcm2,
    0,
])
...
@pytest.mark.parametrize('omega_carrier', [
    1 * u.atomic_angular_frequency,
    0,
])
...
def test_cannot_construct_gaussian_and_sech_pulses_with_negative_omega_carrier(pulse_type):
    with pytest.raises(ion.exceptions.InvalidPotentialParameter):
