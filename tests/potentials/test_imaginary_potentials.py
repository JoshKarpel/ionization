...
class TestImaginaryGaussianRing:
    ...
    def test_decay_time_can_be_positive(self, decay_time):
        ...
        potentials.ImaginaryGaussianRing(decay_time=decay_time)
    ...
    def test_center_can_be_non_negative(self, center):
        potentials.ImaginaryGaussianRing(center=center)
    ...
    def test_width_can_be_positive(self, width):
        ...
        potentials.ImaginaryGaussianRing(width=width)
    ...
    def test_decay_time_can_be_real(self):
        potentials.ImaginaryGaussianRing(decay_time=100 * u.asec)

    def test_decay_time_cannot_have_imaginary_part(self):
        with pytest.raises(exceptions.InvalidPotentialParameter):
            potentials.ImaginaryGaussianRing(decay_time=(100 + 100j) * u.asec)

    def test_decay_time_cannot_be_imaginary(self):
        with pytest.raises(exceptions.InvalidPotentialParameter):
            potentials.ImaginaryGaussianRing(decay_time=100j * u.asec)

    def test_potential_is_imaginary_everywhere(self):
        pot = potentials.ImaginaryGaussianRing()
