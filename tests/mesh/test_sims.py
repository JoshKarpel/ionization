...
class TestSphericalHarmonicSimulation:
    ...
    def test_initial_wavefunction_is_normalized_with_numeric_eigenstates(self, initial_state):
    ...
    @pytest.mark.parametrize(
        'operators, evolution_method', [
            (ion.mesh.SphericalHarmonicLengthGaugeOperators(), ion.mesh.AlternatingDirectionImplicit()),
            (ion.mesh.SphericalHarmonicLengthGaugeOperators(), ion.mesh.SplitInteractionOperator()),
            (ion.mesh.SphericalHarmonicVelocityGaugeOperators(), ion.mesh.SplitInteractionOperator()),
        ],
    )
    def test_with_no_potential_final_state_is_initial_state_with_numeric_eigenstates(self, initial_state, operators, evolution_method):
        sim = ion.mesh.SphericalHarmonicSpecification(
            'test',
            initial_state = initial_state,
            operators = operators,
