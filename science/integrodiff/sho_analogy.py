...
def new_amplitude(initial_amplitude,
                  initial_phase,
                  kick_amplitude,
    ...
    return np.sqrt((initial_amplitude ** 2) + (amp ** 2) + (2 * initial_amplitude * amp * np.cos(initial_phase)))


def new_phase(initial_amplitude,
              initial_phase,
              kick_amplitude,
              test_mass = electron_mass,
              test_charge = proton_charge,
...
if __name__ == '__main__':
    with si.utils.LogManager('simulacra', 'ionization', stdout_level = logging.DEBUG) as logger:
        ...
        initial_phase = 0
        kick_amplitude = .3 * time_field_unit
        ...
        si.vis.xy_plot(
            ...
            x_label = r'$\phi_x$', x_unit = 'rad',
            y_label = r'$x_1$', y_unit = 'bohr_radius',
            **PLOT_KWARGS,
        )
