    ...
    with si.utils.LogManager(
        ...
        def integrand_no_phi(t_inner, t_outer):
            ...
            envelopes = electric_field.get_electric_field_envelope(
                t_outer
            ) * electric_field.get_electric_field_envelope(t_inner)
        ...
        def integrand_phi(t_inner, t_outer):
            ...
            kernel = ide.gaussian_kernel_LEN(t_outer - t_inner, tau_alpha=tau_alpha)
            cosines = np.cos(
                (electric_field.omega_carrier * (t_outer + t_inner))
                + (2 * electric_field.phase)
            )
        ...
        approx_no_phi, error = complex_dblquad(
            integrand_no_phi, t_lower, t_upper, lambda t: t_lower, lambda t: t_upper
        )
        approx_phi, error = complex_dblquad(
            integrand_phi, t_lower, t_upper, lambda t: t_lower, lambda t: t_upper
        )
        ...
        final = 1 + ((0.25 * prefactor) * (approx_no_phi + approx_phi))
