...
if __name__ == '__main__':
    with logman as logger:
        ...
        for cep in tqdm(phases):
            wave = ion.SineWave.from_wavelength(wavelength = 800 * nm, amplitude = .0001 * atomic_electric_field, phase = cep)

            times = np.linspace(-t_bound * wave.period, t_bound * wave.period, 1e4)

            window = ion.SymmetricExponentialTimeWindow(window_time = p_bound * wave.period, window_width = .2 * wave.period)
            wave = ion.SineWave.from_wavelength(wavelength = 800 * nm, amplitude = .0001 * atomic_electric_field, phase = cep, window = window)
