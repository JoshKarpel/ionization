...
def make_plot(args):
    ...
    spec = ion.SphericalHarmonicSpecification(
        ...
        mask = ion.RadialCosineMask(inner_radius = 225 * bohr_radius, outer_radius = 250 * bohr_radius),
        checkpoints = True,
        checkpoint_every = 50,
...
if __name__ == '__main__':
    with logman as logger:
        ...
        fluences = np.array([.01, .1, 1, 2, 5, 10]) * Jcm2
        phases = [0, pi / 4, pi / 2]
        pulse_types = [
            ion.SincPulse,
            ion.GaussianPulse,
        ...
        si.utils.multi_map(make_plot, list(itertools.product(pulse_types, pulse_widths, fluences, phases)), processes = 4)
