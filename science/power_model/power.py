...
def get_power(pulse, times, lookback=None):
    ...
    velocity = (
        -CHARGE * pulse.get_vector_potential_amplitude_numeric_cumulative(times) / MASS
    )
...
def plot_tdse_initial_state_overlap(sims):
    ...
    ionization_rate = [
        np.gradient(s.state_overlaps_vs_time[s.spec.initial_state], s.data_times)
        for s in sims
    ]
...
def plot_tdse_energy_and_power_expectation_values(sims):
    ...
    total_powers = [
        np.gradient(s.total_energy_expectation_value_vs_time, s.data_times)
        for s in sims
    ]
    internal_powers = [
        np.gradient(s.internal_energy_expectation_value_vs_time, s.data_times)
        for s in sims
    ]
...
def plot_tdse_z_and_vz_expectation_values(sims):
    z = [
        s.electric_dipole_moment_expectation_value_vs_time / s.spec.test_charge
        for s in sims
    ]
    ...
    force_times_v_z = [
        sim.spec.test_charge
        * sim.spec.electric_potential.get_electric_field_amplitude(sim.data_times)
        * v
...
def plot_power_model(pulses, times):
    ...
    bs_with_window = [
        solve_power_model(pulse, times, lookback=150 * u.asec) for pulse in pulses
    ]
...
if __name__ == "__main__":
    with LOGMAN as logger:
        ...
        for pulse in pulses:
            pulse.window = window
        times = np.linspace(
            -35 * pulses[0].pulse_width, 35 * pulses[0].pulse_width, 10000
        )
