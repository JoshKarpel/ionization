...
def version_2():
    ...
    fluences = sorted(jp_hyd.parameter_set('fluence').union(jp_ide.parameter_set('fluence')))
    ...
    line_kwargs_hyd = {(phase, fluence): {'linestyle': phase_to_style[phase], 'color': fluence_to_color[fluence], **extra_line_kwargs}
                       for phase, fluence in results_by_phase_and_fluence__hyd.keys()}
    line_kwargs_ide = {(phase, fluence): {'linestyle': phase_to_style[phase], 'color': fluence_to_color[fluence], **extra_line_kwargs}
                       for phase, fluence in results_by_phase_and_fluence__ide.keys()}
    ...
    for metric, log_y in itertools.product(metrics, [False, True]):
        ...
        with si.vis.FigureManager(f'hyd_ide_comparison__pulse_width_scan__sinc__{metric}' + postfix, aspect_ratio = 1, **PLOT_KWARGS) as figman:
            fig = figman.fig
            ...
            ax_hyd.tick_params(labeltop = True, labelright = True, labelbottom = False, labelleft = True, bottom = False)
            ax_ide.tick_params(labeltop = False, labelright = True, labelbottom = True, labelleft = True, top = False)

            legend = ax_hyd.legend(bbox_to_anchor = (1.15, 1), loc = 'upper left', borderaxespad = 0., handles = legend_handles)
...
def version_3():
    ...
    hyd_cos = {fluence: results_by_phase_and_fluence__hyd[0, fluence] for fluence in fluences}
    hyd_sin = {fluence: results_by_phase_and_fluence__hyd[pi / 2, fluence] for fluence in fluences}
    ide_cos = {fluence: results_by_phase_and_fluence__ide[0, fluence] for fluence in fluences}
    ide_sin = {fluence: results_by_phase_and_fluence__ide[pi / 2, fluence] for fluence in fluences}
    ...
    line_kwargs = {fluence: {'color': fluence_to_color[fluence], **extra_line_kwargs}
    ...
    for metric, log_y in itertools.product(metrics, [False, True]):
        ...
        with si.vis.FigureManager(f'hyd_ide_sine_cosine_diff__pulse_width_scan__sinc__{metric}' + postfix, aspect_ratio = 1, **PLOT_KWARGS) as figman:
            fig = figman.fig
            ...
            legend = ax_hyd.legend(bbox_to_anchor = (1.15, 1), loc = 'upper left', borderaxespad = 0.)
