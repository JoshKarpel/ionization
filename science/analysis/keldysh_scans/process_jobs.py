...
PLOT_KWARGS = dict(
    fig_width = si.vis.points_to_inches(500),
)


def get_keldysh_parameter(sim_result, selector = 'bandwidth'):
    return round(sim_result.electric_potential[0].keldysh_parameter(keldysh_omega_selector = selector), 3)


if __name__ == '__main__':
    with si.utils.LogManager('simulacra', 'ionization') as logger:
        states = list(ion.HydrogenBoundState(n, 0) for n in range(1, 10))
        transition_energies = set(np.abs(a.energy - b.energy) for a, b in itertools.product(states, repeat = 2))
        ...
        jp_names = [
            'hyd__keldysh_scan__sinc.job',
            'hyd__keldysh_scan__gaussian.job',
        ]
        for jp_name in jp_names:
            ...
            keldysh_parameters = sorted(set(get_keldysh_parameter(r, selector = selector) for r in jp.data.values()))
            print(phases)
            print(pulse_widths)
            ...
            for metric in metrics:
                for keldysh_parameter in keldysh_parameters:
                    results = [r for r in jp.data.values() if get_keldysh_parameter(r, selector = selector) == keldysh_parameter]
