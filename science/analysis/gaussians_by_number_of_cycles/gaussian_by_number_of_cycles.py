...
def scan_plots_absolute__separate():
    ...
    results_by_flu_cep = {
        (cep, flu): jp.select_by_kwargs(fluence=flu, phase=cep)
        for cep in phases
        for flu in fluences
    }
    ...
    results_by_cyc = {
        cyc: {
            (cep, flu): list(
                r for r in results_by_flu_cep[cep, flu] if get_n_cycles(r) == cyc
            )
            for cep in phases
            for flu in fluences
        }
        ...
        si.vis.xxyy_plot(
            ...
            [
                *[
                    ...
                    for results in results_by_cyc[cyc].values()
                ]
            ],
            [
                *[
            ...
            title=fr"Pulse Width Scan Comparison: Gaussian Pulse, $N_c = {cyc}$",
            title_offset=1.075,
...
def scan_plots_absolute__combined():
    ...
    for metric, log_y in itertools.product(metrics, [False, True]):
        ...
        with si.vis.FigureManager(
            f"GaussianPulseWidthScan__Nc=combined__{metric}" + postfix,
            aspect_ratio=0.8,
            **PLOT_KWARGS,
        ) as figman:
            fig = figman.fig
            ...
            suptitle = fig.suptitle(
                "Pulse Width Scan Comparison: Gaussian Pulse", fontsize=16
            )
            # suptitle.set_x(.6)
            ...
            # LIMITS
            for ax in axes:
                ax.set_ylim(-0.025 if not log_y else 1e-7, 1.025)
                ax.set_xlim(0, 800)
