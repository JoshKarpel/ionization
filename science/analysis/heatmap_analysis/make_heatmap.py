...
def make_modulation_depth_heatmaps(jp):
    ...
    for metric in jp.ionization_metrics:
        for log_z in (True, False):
            ...
            results_sin = {(r.pulse_width, r.fluence): getattr(r, metric) for r in jp.select_by_kwargs(phase = u.pi / 2)}
            ...
            si.vis.xyz_plot(
                f'{jp.name}_modulation_depth_heatmap__{metric}' + ("_logZ" if log_z else ""),
                x_mesh,
                y_mesh,
                z_mesh,
                x_unit = 'asec',
