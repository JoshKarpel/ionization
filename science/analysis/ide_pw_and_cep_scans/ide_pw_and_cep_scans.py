    ...
    for metric in METRICS:
        for plot_attr, line_attr, scan_attr in itertools.permutations(grouping_attrs):
            ...
            for plot_attr_value in jp.parameter_set(plot_attr):
                ...
                results = {(getattr(r, line_attr), getattr(r, scan_attr)): r
                           for r in jp.select_by_kwargs(**{plot_attr: plot_attr_value})}

                plot_unit, plot_unit_tex = get_unit_value_and_latex_from_unit(units[plot_attr])
                line_unit, line_unit_tex = get_unit_value_and_latex_from_unit(units[line_attr])

                for log_y in [False, True]:
                    si.vis.xxyy_plot(
                        ('logY_' if log_y else '') + f'{plot_attr}={uround(plot_attr_value, units[plot_attr])}_{line_attr}_{scan_attr}',
                        [
                            *[[getattr(results[line_attr_value, scan_attr_value], scan_attr)
                               for scan_attr_value in scan_attr_values]
                              for line_attr_value in line_attr_values]
                        ],
                        [
                            *[[getattr(results[line_attr_value, scan_attr_value], metric)
                               for scan_attr_value in scan_attr_values]
                              for line_attr_value in line_attr_values]
                        ],
                        line_labels = [rf'$ {symbols[line_attr]} = {uround(line_attr_value, units[line_attr])} \, {line_unit_tex} $'
                                       for line_attr_value in line_attr_values],
