    ...
    for metric in METRICS:
        for cep in ceps:
            for log_z in (True, False):
                logger.debug(f'Making plot for CEP {uround(cep, pi)}pi, metric {metric}')

                results = {(r.pulse_width, r.fluence): getattr(r, metric) for r in jp.select_by_kwargs(phase = cep)}
...
def make_modulation_depth_heatmaps(jp):
    ...
    for metric in METRICS:
        for log_z in (True, False):
            ...
            results_cos = {(r.pulse_width, r.fluence): getattr(r, metric) for r in jp.select_by_kwargs(phase = 0)}
            results_sin = {(r.pulse_width, r.fluence): getattr(r, metric) for r in jp.select_by_kwargs(phase = pi / 2)}
...
if __name__ == '__main__':
    with LOGMAN as logger:
        jp = clu.JobProcessor.load(os.path.join(os.getcwd(), 'job_processors', 'hyd__heatmap_PW_vs_FLU_2ceps.job'))
