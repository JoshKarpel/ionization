...
if __name__ == "__main__":
    with logman as logger:
        ...
        animator_kwargs = dict(target_dir=OUT_DIR, length=30, fps=30, fig_dpi_scale=2)
        ...
        animators = [
            ...
            animation.animators.PolarAnimator(
                ...
                axman_wavefunction=animation.animators.SphericalHarmonicPhiSliceMeshAxis(
                    which="g2", plot_limit=plot_radius * bohr_radius, shading=shading
                ),
                axman_lower_right=deepcopy(axman_lower_right),
                axman_upper_right=deepcopy(axman_upper_right),
                **animator_kwargs,
        ...
        for amplitude, cycle, gauge in itertools.product(amplitudes, cycles, gauges):
            electric_field = ion.SineWave.from_photon_energy(
                np.abs(state_a.energy - state_b.energy),
                amplitude=amplitude * atomic_electric_field,
            )

            rabi_frequency = (
                amplitude * atomic_electric_field * dipole_moment / hbar / twopi
            )
