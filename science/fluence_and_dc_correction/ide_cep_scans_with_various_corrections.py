...
if __name__ == "__main__":
    with LOGMAN as logger:
        jp_plain = clu.JobProcessor.load(
            "job_processors/PAPER_OL_ide_cep_scan_plain.job"
        )
        jp_dc = clu.JobProcessor.load(
            "job_processors/PAPER_OL_ide_cep_scan_dc_correct.job"
        )
        jp_dc_and_flu = clu.JobProcessor.load(
            "job_processors/PAPER_OL_ide_cep_scan_dc_and_flu_correct.job"
        )

        jp_to_label = {jp_dc: "dc", jp_plain: "plain", jp_dc_and_flu: "dc and flu"}
        ...
        fluences = np.array(
            sorted(
                set.intersection(*[jp.parameter_set("fluence") for jp in jp_to_label])
            )
        )
        pulse_widths = set.intersection(
            *[jp.parameter_set("pulse_width") for jp in jp_to_label]
        )
        pulse_widths.remove(400 * u.asec)
        pulse_widths = np.array(sorted(pulse_widths))
        phases = np.array(
            sorted(set.intersection(*[jp.parameter_set("phase") for jp in jp_to_label]))
        )
        ...
        for fluence, pulse_width in itertools.product(fluences, pulse_widths):
                ...
                jp: sorted(
                    [
                        ...
                        for r in jp.select_by_kwargs(
                            fluence=fluence, pulse_width=pulse_width
                        )
                        if r.pulse_width in pulse_widths
                    ],
            ...
            si.vis.xxyy_plot(
                ...
                y_data=[
                    [r.final_initial_state_overlap for r in results]
            ...
            si.vis.xxyy_plot(
                ...
                y_data=[
                    [
                        ...
                        - plain_r.final_initial_state_overlap
