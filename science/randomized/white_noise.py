...
if __name__ == '__main__':
    with LOGMAN as logger:
        ...
        times = np.linspace(-t_bound * pw, t_bound * pw, int(uround(pw, asec)) * 2 * t_bound)
        ...
        si.vis.xy_plot(
            ...
            times,
            white_noise_field,
            x_unit = 'asec', x_label = r'$t$',
            **PLOT_KWARGS,
        )

        white_noise_autocorrelation = np.correlate(white_noise_field, white_noise_field, 'same')
        ...
        lowcut = sinc_pulse.frequency_min
        highcut = sinc_pulse.frequency_max
        keep_indices = np.greater_equal(np.abs(fft_freq), lowcut) * np.less_equal(np.abs(fft_freq), highcut)
