...
if __name__ == "__main__":
    with LOGMAN as logger:
        ...
        vector_potential = pulse.get_vector_potential_amplitude_numeric_cumulative(
            times
        )
        vector_potential = interp.CubicSpline(
            x=times, y=vector_potential, bc_type="clamped"
        )

        quiver_motion = (electron_charge / electron_mass) * integ.cumtrapz(
            y=vector_potential(times), x=times, initial=0
        )
