...
def instantaneous_tunneling_rate_from_cc_kernel(pulse, times):
    ...
    for current_time_idx, current_time in enumerate(times):
        ...
        kernel_integrals[current_time_idx] = integ.simps(y=kernel, x=previous_times)

    gamma = (
        2
        * prefactor
        * kernel_integrals
        * (pulse.get_electric_field_amplitude(times) ** 2)
    )
    return gamma


def landau_if_below_critical_field(field_amplitude):
    critical_field = 0.0835 * u.atomic_electric_field
    return np.where(
...
if __name__ == "__main__":
    with LOGMAN as logger:
        ...
        amplitudes = np.array([0.3, 0.5]) * u.atomic_electric_field
        number_of_cycleses = [6, 12]
        omegas = np.array([0.2]) * u.atomic_angular_frequency
        ...
        for amplitude, number_of_cycles, omega in itertools.product(
            ...
            times = np.linspace(
                -t_extra_lower * pulse.pulse_center,
                (2 + t_extra_upper) * pulse.pulse_center,
                5000,
            )
            ...
            figman = si.vis.xy_plot(
                ...
                * u.proton_charge
                / u.atomic_momentum,
                line_kwargs=[{"alpha": 0.6}, {"alpha": 0.6}],
                x_label="$t$",
                x_unit=x_unit_name,
            ...
            cc_tunneling_rate = instantaneous_tunneling_rate_from_cc_kernel(
                pulse, times
            )
            # ax_rate.plot(
            #     times / x_unit,
            ...
            ax_rate.plot(
                times / x_unit,
                -empirical_prefactor
                * ((efield_vs_time / u.atomic_electric_field) ** 2),
                color="C4",
            )
            ...
            ax_fields.set_xlim(
                0 * pulse.pulse_center / x_unit, 2 * pulse.pulse_center / x_unit
            )
            ax_fields.set_ylim(-2, 2)

            legend_handles = [
                mlines.Line2D(
                    [], [], color="C0", alpha=0.6, linewidth=1, label=r"$\mathcal{E}$"
                ),
                mlines.Line2D(
                    [], [], color="C1", alpha=0.6, linewidth=1, label=r"$\mathcal{A}$"
                ),
                mlines.Line2D(
                    [],
                    [],
            ...
            ax_fields.legend(loc="upper left", fontsize=10, handles=legend_handles)
