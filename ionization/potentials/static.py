...
class SoftCoulombPotential(potential.PotentialEnergy):
    ...
    def __init__(
        ...
        softening_distance: float = 0.05 * u.bohr_radius,
    ):
        """
        Parameters
        ----------
    ...
    def __call__(self, *, r, test_charge, **kwargs):
        ...
        return (
            ...
            * self.charge
            * test_charge
            / np.sqrt((r ** 2) + (self.softening_distance ** 2))
        )
    ...
    def info(self) -> si.Info:
        ...
        info.add_field(
            "Softening Distance",
            utils.fmt_quantity(self.softening_distance, utils.LENGTH_UNITS),
        )
...
class HarmonicOscillator(potential.PotentialEnergy):
    ...
    def from_frequency_and_mass(
        cls, omega: float = 1.5192675e15 * u.Hz, mass: float = u.electron_mass, **kwargs
    ) -> "HarmonicOscillator":
        """
        Parameters
        ----------
    ...
    def from_ground_state_energy_and_mass(
        ...
        **kwargs,
        ...
        return cls.from_frequency_and_mass(
            omega=2 * ground_state_energy / u.hbar, mass=mass, **kwargs
        )

    @classmethod
    def from_energy_spacing_and_mass(
        cls, energy_spacing: float = 1 * u.eV, mass: float = u.electron_mass, **kwargs
    ) -> "HarmonicOscillator":
        """
        ...
        """
        return cls.from_frequency_and_mass(
            omega=energy_spacing / u.hbar, mass=mass, **kwargs
        )
    ...
    def __call__(self, *, r, **kwargs):
        ...
        if self.cutoff_distance is not None:
            ...
            return np.where(
                np.less_equal(np.abs(centered_r), self.cutoff_distance), inside, outside
            )
        else:
            return inside
    ...
    def info(self) -> si.Info:
        ...
        info.add_field(
            "Spring Constant",
            utils.fmt_quantity(self.spring_constant, utils.FORCE_UNITS),
        )
...
class FiniteSquareWell(potential.PotentialEnergy):
    ...
    def info(self) -> si.Info:
        ...
        info.add_field(
            "Potential Depth",
            utils.fmt_quantity(self.potential_depth, utils.ENERGY_UNITS),
        )
...
class GaussianPotential(potential.PotentialEnergy):
    ...
    def info(self) -> si.Info:
        ...
        info.add_field(
            "Potential Depth",
            utils.fmt_quantity(self.potential_extrema, utils.ENERGY_UNITS),
        )
        info.add_field("Width", utils.fmt_quantity(self.width, utils.LENGTH_UNITS))
        info.add_field("Center", utils.fmt_quantity(self.center, utils.LENGTH_UNITS))
...
class GaussianScatterer(potential.PotentialEnergy):
    ...
    def __call__(self, *, z, x, **kwargs):
        ...
        gaussian *= np.exp(-0.5 * ((centered_z / self.z_width) ** 2))
        gaussian *= np.exp(-0.5 * ((centered_x / self.x_width) ** 2))
    ...
    def info(self):
        ...
        info.add_field(
            "Z Center", utils.fmt_quantity(self.z_center, utils.LENGTH_UNITS)
        )
        info.add_field(
            "X Center", utils.fmt_quantity(self.x_center, utils.LENGTH_UNITS)
        )
...
class LogisticScatterer(potential.PotentialEnergy):
    ...
    def __init__(
        self,
        potential_extrema: float = 100 * u.eV,
        z_width: float = 0.2 * u.nm,
        x_width: float = 0.2 * u.nm,
        z_extent=1 * u.nm,
        x_extent=1 * u.nm,
        z_center: float = 0,
        x_center: float = 0,
    ...
    def __call__(self, *, z, x, **kwargs):
        ...
        pot *= 1 / (1 + np.exp(-(centered_x + self.x_extent) / self.x_width)) - 1 / (
            1 + np.exp(-(centered_x - self.x_extent) / self.x_width)
        )
        pot *= 1 / (1 + np.exp(-(centered_z + self.z_extent) / self.z_width)) - 1 / (
            1 + np.exp(-(centered_z - self.z_extent) / self.z_width)
        )
    ...
    def info(self):
        ...
        info.add_field(
            "Z Extent", utils.fmt_quantity(self.z_extent, utils.LENGTH_UNITS)
        )
        info.add_field(
            "X Extent", utils.fmt_quantity(self.x_extent, utils.LENGTH_UNITS)
        )
