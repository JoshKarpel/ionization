...
class Kernel(ABC):
    ...
    @abstractmethod
    def __call__(
        self, current_time, previous_time, electric_potential, vector_potential
    ):
        """
        Parameters
        ----------
...
class LengthGaugeHydrogenKernel(Kernel):
    ...
    def __call__(
        self, time_current, time_previous, electric_potential, vector_potential
    ):
    ...
    def evaluate_kernel_function(self, time_difference: float) -> float:
        ...
        with warnings.catch_warnings():
            ...
            kernel = (
                self.kernel_prefactor
                * np.exp(1j * self.omega_bound * time_difference)
                * self.kernel_function(time_difference)
            )
    ...
    def kernel_function(self):
        ...
        integrand = ((k ** 4) / ((1 + ((a * k) ** 2)) ** 6)) * (
            sym.exp(-sym.I * hb * (k ** 2) * td / (2 * m))
        )

        kernel = sym.integrate(integrand, (k, 0, sym.oo))
        kernel = kernel.subs([(a, u.bohr_radius), (m, u.electron_mass), (hb, u.hbar)])
        kernel = kernel.evalf()  # partial evaluation of coefficients

        kernel_func = sym.lambdify(
            (td,), kernel, modules=["numpy", {"erfc": special.erfc}]
        )
...
class LengthGaugeHydrogenKernelWithContinuumContinuumInteraction(
    ...
    def __init__(self, bound_state_energy=states.HydrogenBoundState(1).energy):
        ...
        self.phase_prefactor = (
            -1j * (u.electron_charge ** 2) / (2 * u.electron_mass * u.hbar)
        )

    def __call__(
        self, current_time, previous_time, electric_potential, vector_potential
    ):
        kernel = super().__call__(
            current_time, previous_time, electric_potential, vector_potential
        )

        return kernel * self._vector_potential_phase_factor(
            current_time, previous_time, vector_potential
        )

    def _vector_potential_phase_factor(
        self, current_time, previous_time, vector_potential
    ):
        vp_current = vector_potential(current_time)
        time_difference = current_time - previous_time

        vp_integral = self._integrate_vector_potential_over_previous_times(
            previous_time, vector_potential, power=1
        )
        vp2_integral = self._integrate_vector_potential_over_previous_times(
            previous_time, vector_potential, power=2
        )

        integral = (
            ((vp_current ** 2) * time_difference)
            - (2 * vp_current * vp_integral)
            + vp2_integral
        )
        integral %= (
            u.twopi
        )  # it's a phase, so we're free to do this, which should make the exp more stable

        return np.exp(self.phase_prefactor * integral)

    def _integrate_vector_potential_over_previous_times(
        self, previous_time, vector_potential, power=1
    ):
