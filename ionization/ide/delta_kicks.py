...
from .. import core, potentials, vis
...
def decompose_potential_into_kicks(
    electric_potential: potentials.ElectricPotential, times: np.array
) -> DeltaKicks:
    """
...
class DeltaKickSimulation(si.Simulation):
    ...
    def __init__(self, spec):
        ...
        if self.spec.electric_potential_dc_correction and not isinstance(
            ...
            logger.warning(
                f"Replaced electric potential {old_pot} --> {self.spec.electric_potential} for {self}"
            )

        if not isinstance(self.spec.electric_potential, DeltaKicks):
            self.spec.kicks = decompose_potential_into_kicks(
                self.spec.electric_potential, self.times
            )
        else:
        ...
        self.data_times = np.array(
            [self.spec.time_initial]
            + [k.time for k in self.spec.kicks]
            + [self.spec.time_final]
        )  # for consistency with other simulations
    ...
    def _solve(self):
        ...
        for t_idx, kick in enumerate(self.spec.kicks, start=1):
            history_sum = (self.spec.integral_prefactor * kick.amplitude) * np.sum(
                amplitudes[: t_idx - 1]
                * self.evaluate_kernel(self.data_times[t_idx], self.data_times[1:t_idx])
                * self.b[1:t_idx]
            )
            self.b[t_idx] = (self.b[t_idx - 1] + history_sum) / (
                1 - (self.spec.integral_prefactor * k0 * (kick.amplitude ** 2))
            )
