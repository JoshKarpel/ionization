...
def format_log_str(is_logs, axis_names):
    ...
    if any(is_logs):
        log_str = "__log" + "".join(
            axis_name for is_log, axis_name in zip(is_logs, axis_names) if is_log
        )
...
class PulseParameterScanMixin:
    ...
    def _select_two_parameters(self, parameters=None):
        ...
        while True:
            ...
            other_parameters = [
                p for p in parameters if p not in (first_parameter, second_parameter)
            ]
    ...
    def _get_lines(
        self, line_parameter, scan_parameter, ionization_metric, base_selector
    ):
        x_data = []
        y_data = []
        line_labels = []
        ...
        for line_parameter_value in sorted(self.parameter_set(line_parameter)):
            selector = {**base_selector, line_parameter: line_parameter_value}
            results = sorted(
                self.select_by_kwargs(**selector),
                key=lambda result: getattr(result, scan_parameter),
            )

            x_data.append(
                np.array([getattr(result, scan_parameter) for result in results])
            )
            y_data.append(
                np.array([getattr(result, ionization_metric) for result in results])
            )
    ...
    def make_scans(self):
        for (
            ...
            scan_parameter,
            plot_parameters,
        ) in self._select_two_parameters():
            try:
                line_parameter_set, scan_parameter_set = (
                    self.parameter_set(line_parameter),
                    self.parameter_set(scan_parameter),
                )
            ...
            if any(
                (
                    len(scan_parameter_set)
                    < 10,  # not enough x data, or too many lines
                    len(line_parameter_set) > 8,
                )
            ):
                continue

            plot_parameter_to_set = self._get_parameters_to_parameter_sets(
                plot_parameters
            )
            if (
                len(plot_parameter_to_set) == 0
            ):  # this sometimes happens on heatmap jobs?
                continue

            plot_parameters = tuple(plot_parameter_to_set.keys())
            plot_parameter_value_groups = (
                plot_parameter_values
                for plot_parameter_values in itertools.product(
                    *plot_parameter_to_set.values()
                )
            )
            ...
            for plot_parameter_values in plot_parameter_value_groups:
                for ionization_metric in self.ionization_metrics:
                    ...
                    x_data, y_data, line_labels = self._get_lines(
                        line_parameter, scan_parameter, ionization_metric, base_selector
                    )
                    if any(x[0] == x[-1] for x in x_data):
                        continue

                    str_for_filename, str_for_plot = self._get_plot_parameter_strings(
                        plot_parameters, plot_parameter_values
                    )
                    filename = "__".join(
                        (
                            self.name,
                    ...
                    for log_x, log_y in itertools.product((True, False), repeat=2):
                        if any(
                            (
                                log_x and scan_parameter == "phase",
                        ...
                        log_str = format_log_str((log_x, log_y), ("X", "Y"))
    ...
    def make_heatmaps(self):
        for x_parameter, y_parameter, plot_parameters in self._select_two_parameters():
            try:
                x_parameter_set, y_parameter_set = (
                    self.parameter_set(x_parameter),
                    self.parameter_set(y_parameter),
                )
            except AttributeError:
                continue

            if any(
                (len(x_parameter_set) < 10, len(y_parameter_set) < 10)
            ):  # not dense enough
                continue
            ...
            for plot_parameter_values in plot_parameter_value_groups:
                results = self.select_by_kwargs(
                    **dict(zip(plot_parameters, plot_parameter_values))
                )
                xy_to_result = {
                    (getattr(r, x_parameter), getattr(r, y_parameter)): r
                    for r in results
                }

                for ionization_metric in self.ionization_metrics:
                    xy_to_metric = {
                        (x, y): getattr(r, ionization_metric)
                        for (x, y), r in xy_to_result.items()
                    }
                    ...
                    for log_x, log_y, log_z in itertools.product(
                        (True, False), repeat=3
                    ):
                        if any(
                            (
                                log_x and x_parameter == "phase",
                                log_y
                                and y_parameter == "phase",  # skip log phase plots
                        ...
                        log_str = format_log_str((log_x, log_y, log_z), ("X", "Y", "Z"))
    ...
    def make_modulation_depth_heatmaps(self):
        ...
        scan_parameters = [
            s for s in self.scan_parameters if s != "phase"
        ]  # no scans over phase
        for x_parameter, y_parameter, plot_parameters in self._select_two_parameters(
            ...
            for plot_parameter_values in plot_parameter_value_groups:
                ...
                for ionization_metric in self.ionization_metrics:
                    xy_to_metric_cos = {
                        (x, y): getattr(r, ionization_metric)
                        for (x, y), r in xy_to_result_cos.items()
                    }
                    xy_to_metric_sin = {
                        (x, y): getattr(r, ionization_metric)
                        for (x, y), r in xy_to_result_sin.items()
                    }
                    ...
                    try:
                        for ii, x_value in enumerate(x):
                            for jj, y_value in enumerate(y):
                                z_mesh[ii, jj] = modulation_depth(
                                    xy_to_metric_cos[x_value, y_value],
                                    xy_to_metric_sin[x_value, y_value],
                                )
                    except KeyError:  # alignment problem, job is probably not actually a heatmap over these parameters
                        continue
...
class MeshSimulationResult(PulseSimulationResult):
    def __init__(self, sim, job_processor):
        ...
        self.final_norm = copy(sim.data.norm[-1])
        self.final_initial_state_overlap = copy(
            state_overlaps[sim.spec.initial_state][-1]
        )
        self.final_bound_state_overlap = copy(
            sum(state_overlaps[s][-1] for s in sim.bound_states)
        )
        self.final_free_state_overlap = copy(
            sum(state_overlaps[s][-1] for s in sim.free_states)
        )

        try:
            self.final_internal_energy_expectation_value = copy(
                sim.internal_energy_expectation_value_vs_time[-1]
            )
            self.final_total_energy_expectation_value = copy(
                sim.total_energy_expectation_value_vs_time[-1]
            )
        except AttributeError:
            pass

        try:
            self.final_radial_position_expectation_value = copy(
                sim.radial_position_expectation_value_vs_time[-1]
            )
        except AttributeError:
            pass

        try:
            self.final_electric_dipole_moment_expectation_value = copy(
                sim.electric_dipole_moment_expectation_value_vs_time[-1]
            )
        except AttributeError:
            pass
    ...
    def make_wavefunction_plots(self, sim):
        plot_kwargs = dict(target_dir=self.plots_dir, plot_name="name", show_title=True)

        grouped_states, group_labels = sim.group_free_states_by_continuous_attr(
            "energy", divisions=12, cutoff_value=100 * u.eV, attr_unit="eV"
        )
...
class MeshJobProcessor(PulseJobProcessor):
    ...
    def make_velocity_plot(self):
        sim_numbers = [
            result.file_name for result in self.data.values() if result is not None
        ]
        velocity = np.array(
            [
                result.spacetime_points / result.running_time
        ...
        si.vis.xy_plot(
            ...
            y_label="Space-Time Points / Second",
...
class MeshConvergenceJobProcessor(MeshJobProcessor):
    ...
    def make_summary_plots(self):
        ...
        if len(self.unprocessed_sim_names) == 0:
            logger.info(
                f"Generating relative pulse parameter scans for job {self.name}"
            )
    ...
    def make_scans(self):
        for ionization_metric in self.ionization_metrics:
            ...
            for plot_parameter, scan_parameter in itertools.permutations(
                self.scan_parameters
            ):
                plot_parameter_name, scan_parameter_name = (
                    plot_parameter.replace("_", " ").title(),
                    scan_parameter.replace("_", " ").title(),
                )
                plot_parameter_unit, scan_parameter_unit = (
                    PARAMETER_TO_UNIT_NAME[plot_parameter],
                    PARAMETER_TO_UNIT_NAME[scan_parameter],
                )
                plot_parameter_set, scan_parameter_set = (
                    self.parameter_set(plot_parameter),
                    self.parameter_set(scan_parameter),
                )
                ...
                for plot_parameter_value in plot_parameter_set:
                    ...
                    selector = {plot_parameter: plot_parameter_value}
                    results = sorted(
                        self.select_by_kwargs(**selector),
                        key=lambda result: getattr(result, scan_parameter),
                    )
                    ...
                    line = np.array(
                        [getattr(result, ionization_metric) for result in results]
                    )

                    x = np.array(
                        [getattr(result, scan_parameter) for result in results]
                    )
            ...
            for plot_parameter, scan_parameter in itertools.permutations(
                ...
                for plot_parameter_value in plot_parameter_set:
                    ...
                    line = np.array(
                        [
                            np.abs(
                                getattr(result, ionization_metric)
                                - getattr(results[0], ionization_metric)
                            )
    ...
    def make_heatmaps(self):
        for ionization_metric in self.ionization_metrics:
            ...
            for x_parameter, y_parameter in itertools.combinations(
                self.scan_parameters, r=2
            ):
                x_parameter_name, y_parameter_name = (
                    x_parameter.replace("_", " ").title(),
                    y_parameter.replace("_", " ").title(),
                )
                x_parameter_unit, y_parameter_unit = (
                    PARAMETER_TO_UNIT_NAME[x_parameter],
                    PARAMETER_TO_UNIT_NAME[y_parameter],
                )
                x_parameter_set, y_parameter_set = (
                    self.parameter_set(x_parameter),
                    self.parameter_set(y_parameter),
                ...
                xy_to_metric = {
                    (getattr(r, x_parameter), getattr(r, y_parameter)): getattr(
                        r, ionization_metric
                    )
