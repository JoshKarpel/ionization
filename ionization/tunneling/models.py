...
class TunnelingModel(abc.ABC):
    ...
    def tunneling_rate(
        self, electric_field_amplitude: float, ionization_potential: float
    ) -> float:
        """
        ...
        """
        rate = self._tunneling_rate(
            electric_field_amplitude=electric_field_amplitude,
            ionization_potential=ionization_potential,
        )
        rate = np.where(
            np.abs(electric_field_amplitude) <= self.upper_amplitude_cutoff, rate, 0
        ).squeeze()
    ...
    def info(self) -> si.Info:
        ...
        info.add_field(
            ...
            utils.fmt_quantity(self.upper_amplitude_cutoff, utils.ELECTRIC_FIELD_UNITS),
        )
...
class LandauRate(TunnelingModel):
    ...
    def _tunneling_rate(self, electric_field_amplitude, ionization_potential):
        ...
        with warnings.catch_warnings():
            ...
            rate = (
                -0.5
                * (4 * (2 * scaled_potential) / f)
                * np.exp(-2 / (3 * f))
                / u.atomic_time,
            )

        rate = np.where(np.isclose(scaled_amplitude, 0), 0, rate)
...
class PosthumusRate(TunnelingModel):
    ...
    def _tunneling_rate(self, electric_field_amplitude, ionization_potential):
        ...
        T = (
            u.pi
            * self.atomic_number
            / (scaled_potential * np.sqrt(2 * scaled_potential))
        )
        ...
        with warnings.catch_warnings():
            ...
            rate = (
                ...
                * (
                    ...
                    - (
                        (scaled_potential ** 2)
                        / (4 * self.atomic_number * scaled_amplitude)
                    )
                )
                / (2 * T)
                / u.atomic_time
        ...
        rate = np.where(
            np.less_equal(scaled_amplitude, u.atomic_electric_field / 16), 0, rate
        )
...
class MulserRate(TunnelingModel):
    ...
    def _tunneling_rate(self, electric_field_amplitude, ionization_potential):
        ...
        with warnings.catch_warnings():
            ...
            C = (
                -u.pi
                * ((2 * scaled_potential) ** 0.125)
                * (2 * scaled_potential)
                / (np.sqrt(2) * (scaled_amplitude ** 0.75))
            )
            alpha = 4 * np.sqrt(scaled_amplitude) / ((2 * scaled_potential) ** 0.75)
            beta = (3 + alpha) * C / 4
            A = np.exp(-(7 - (3 * alpha)) * C / 4)

            rate = (
                -0.5
                * (scaled_potential / np.abs(beta))
                * np.log((A + np.exp(np.abs(beta))) / (A + 1))
                / u.atomic_time
            )
...
class ADKRate(TunnelingModel):
    ...
    def _tunneling_rate(self, electric_field_amplitude, ionization_potential):
        ...
        with warnings.catch_warnings():
            ...
            f_lm = (
                ((2 * self.l) + 1)
                * special.factorial(self.l + np.abs(self.m))
                / (
                    (2 ** np.abs(self.m))
                    * special.factorial(np.abs(self.m))
                    * special.factorial(self.l - np.abs(self.m))
                )
            )
            C_nl = ((2 * u.e / self.n_star) ** self.n_star) / np.sqrt(
                u.twopi * self.n_star
            )
            pre = (C_nl ** 2) * f_lm
            ...
            rate = (
                ...
                * scaled_potential
                * np.sqrt(3 * f / u.pi)
                * ((2 / f) ** (2 * self.n_star - np.abs(self.m) - 1))
                * np.exp(-2 / (3 * f))
                / u.atomic_time
            )
...
class ADKExtendedToBSIRate(ADKRate):
    ...
    def _tunneling_rate(self, electric_field_amplitude, ionization_potential):
        ...
        with warnings.catch_warnings():
            ...
            factor_2 = (
                4 * u.e * (scaled_potential ** 1.5) / (scaled_amplitude * self.n_star)
            ) ** (2 * self.n_star)

            def integrand(x, amplitude=0):
                ai, aip, bi, bip = special.airy(
                    (x ** 2) + ((2 * scaled_potential) / ((2 * amplitude) ** 1.5))
                )
                return (x * ai) ** 2

            try:
                integral, *_ = integ.quad(
                    functools.partial(integrand, amplitude=scaled_amplitude), 0, np.inf
                )
            except TypeError:
                integral = np.array(
                    [
                        integ.quad(
                            functools.partial(integrand, amplitude=a), 0, np.inf
                        )[0]
                        for a in scaled_amplitude
                    ]
                )

            rate = -0.5 * pre * factor_1 * factor_2 * integral / u.atomic_time
