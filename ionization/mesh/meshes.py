...
class QuantumMesh(abc.ABC):
    ...
    @abc.abstractmethod
    def wrapping_direction_to_order(
        self, wrapping_direction: Optional[WrappingDirection]
    ) -> Optional[str]:
    ...
    def get_g_with_states_removed(
        self, states: Iterable[StateOrGMesh], g: StateOrGMesh
    ) -> GMesh:
        """
    ...
    def inner_product(self, a: StateOrGMesh = None, b: StateOrGMesh = None) -> complex:
        ...
        return (
            np.sum(np.conj(self.state_to_g(a)) * self.state_to_g(b))
    ...
    def expectation_value(
        self,
        state: StateOrGMesh = None,
        operator: mesh_operators.SumOfOperators = mesh_operators.SumOfOperators(),
    ) -> float:
    ...
    def internal_energy_expectation_value(self, state: StateOrGMesh = None):
        ...
        return self.expectation_value(
            state, operator=self.operators.internal_hamiltonian(self)
        )
    ...
    def total_energy_expectation_value(self, state: StateOrGMesh = None):
        ...
        return self.expectation_value(
            state, operator=self.operators.total_hamiltonian(self)
        )
...
class LineMesh(QuantumMesh):
    ...
    def __init__(self, sim: "sims.MeshSimulation"):
        ...
        self.z_mesh = np.linspace(
            -self.spec.z_bound, self.spec.z_bound, self.spec.z_points
        )
        ...
        if self.spec.use_numeric_eigenstates:
            ...
            self.analytic_to_numeric = self._get_numeric_eigenstate_basis(
                self.spec.number_of_numeric_eigenstates
            )
            self.spec.test_states = sorted(
                list(self.analytic_to_numeric.values()), key=lambda x: x.energy
            )
    ...
    def get_g_for_state(self, state: StateOrGMesh) -> GMesh:
        if state.analytic and self.spec.use_numeric_eigenstates:
            try:
                ...
                logger.debug(
                    f"Analytic to numeric eigenstate lookup failed for state {state}"
                )
    ...
    def _get_numeric_eigenstate_basis(self, number_of_eigenstates: int):
        ...
        eigenvalues, eigenvectors = sparsealg.eigsh(
            h, k=number_of_eigenstates, which="SA"
        )

        for nn, (eigenvalue, eigenvector) in enumerate(
            zip(eigenvalues, eigenvectors.T)
        ):
            eigenvector /= np.sqrt(
                self.inner_product_multiplier * np.sum(np.abs(eigenvector) ** 2)
            )  # normalize
            ...
            try:
                ...
                analytic_state = states.OneDPlaneWave.from_energy(
                    eigenvalue, mass=self.spec.test_mass
                )
        ...
        logger.debug(
            f"Generated numeric eigenbasis with {len(analytic_to_numeric)} states"
        )

        return analytic_to_numeric

    def gauge_transformation(
        self, *, g: GMesh = None, leaving_gauge: core.Gauge
    ) -> GMesh:
        g = self.state_to_g(g)
        if leaving_gauge is None:
            leaving_gauge = self.spec.evolution_gauge

        vamp = self.spec.electric_potential.get_vector_potential_amplitude_numeric_cumulative(
            self.sim.times_to_current
        )
        integral = integ.simps(y=vamp ** 2, x=self.sim.times_to_current)

        dipole_to_velocity = np.exp(
            1j
            * integral
            * (self.spec.test_charge ** 2)
            / (2 * self.spec.test_mass * u.hbar)
        )
        dipole_to_length = np.exp(
            -1j * self.spec.test_charge * vamp[-1] * self.z_mesh / u.hbar
        )
    ...
    def get_mesh_slicer(self, plot_limit: Optional[float]) -> slice:
        if plot_limit is None:
            ...
            mesh_slicer = slice(
                int(self.z_center_index - x_lim_points),
                int(self.z_center_index + x_lim_points + 1),
                1,
            )
...
class RectangleMesh(QuantumMesh):
    ...
    def wrapping_direction_to_order(
        ...
        if wrapping_direction is None:
            ...
            raise ValueError(
                f"{wrapping_direction} is not a valid specifier for flatten_mesh (valid specifiers: 'z', 'x')"
            )
    ...
    def get_mesh_slicer(self, plot_limit: Optional[float]) -> slice:
        if plot_limit is None:
            ...
            mesh_slicer = slice(
                int(self.z_center_index - z_lim_points),
                int(self.z_center_index + z_lim_points + 1),
                1,
            )
...
class CylindricalSliceMesh(QuantumMesh):
    ...
    def __init__(self, sim: "sims.MeshSimulation"):
        ...
        self.z = np.linspace(-self.spec.z_bound, self.spec.z_bound, self.spec.z_points)
        self.rho = np.delete(
            np.linspace(0, self.spec.rho_bound, self.spec.rho_points + 1), 0
        )
    ...
    def wrapping_direction_to_order(
        ...
        if wrapping_direction is None:
            ...
            raise ValueError(
                f"{wrapping_direction} is not a valid specifier for flatten_mesh (valid specifiers: 'z', 'rho')"
            )
    ...
    def get_probability_current_density_vector_field(
        self, state: StateOrGMesh = None
    ) -> VectorMesh:
        """
        ...
        current_density_mesh_z = np.imag(
            np.conj(mesh)
            * mesh_operators.apply_operators_sequentially(self, mesh, [z_op])
        )
        current_density_mesh_rho = np.imag(
            np.conj(mesh)
            * mesh_operators.apply_operators_sequentially(self, mesh, [rho_op])
        )
    ...
    def get_mesh_slicer(self, plot_limit: Optional[float] = None):
        ...
        if plot_limit is None:
            ...
            mesh_slicer = (
                ...
                slice(0, int(rho_lim_points + 1), 1),
            )
...
class SphericalSliceMesh(QuantumMesh):
    ...
    def wrapping_direction_to_order(
        ...
        if wrapping_direction is None:
            ...
            raise ValueError(
                f"{wrapping_direction} is not a valid specifier for flatten_mesh (valid specifiers: 'r', 'theta')"
            )
...
class SphericalHarmonicMesh(QuantumMesh):
    ...
    def __init__(self, sim: "sims.MeshSimulation"):
        ...
        if self.spec.use_numeric_eigenstates:
            self.analytic_to_numeric = self.get_numeric_eigenstate_basis(
                self.spec.numeric_eigenstate_max_energy,
                self.spec.numeric_eigenstate_max_angular_momentum,
            )
            self.spec.test_states = sorted(
                list(self.analytic_to_numeric.values()), key=lambda x: x.energy
            )
            if not self.spec.initial_state.numeric:
                self.spec.initial_state = self.analytic_to_numeric[
    ...
    def wrapping_direction_to_order(
        ...
        if wrapping_direction is None:
            ...
            raise ValueError(
                f"{wrapping_direction} is not a valid specifier for flatten_mesh (valid specifiers: 'l', 'r')"
            )

    def get_g_for_state(self, state: StateOrGMesh) -> GMesh:
        if isinstance(state, states.QuantumState) and all(
            hasattr(s, "spherical_harmonic") for s in state
        ):
            if state.analytic and self.spec.use_numeric_eigenstates:
                try:
                    state = self.analytic_to_numeric[state]
            ...
            for s in state:
                g[s.l, :] += self.get_radial_g_for_state(
                    s
                )  # fill in g state-by-state to improve runtime

            return g
        else:
            raise NotImplementedError(
                "States with non-definite angular momentum components are not currently supported by SphericalHarmonicMesh"
            )
    ...
    def inner_product(self, a: StateOrGMesh = None, b: StateOrGMesh = None) -> complex:
        ...
        """
        if (
            isinstance(a, states.QuantumState)
            and all(hasattr(s, "spherical_harmonic") for s in a)
            and b is None
        ):  # shortcut
            ip = 0
            ...
            for s in a:
                ip += np.sum(
                    np.conj(self.get_radial_g_for_state(s)) * self.g[s.l, :]
                )  # calculate inner product state-by-state to improve runtime
    ...
    def inner_product_with_plane_waves(
        self, thetas, wavenumbers, g: Optional[GMesh] = None
    ):
        """
        ...
        for (ii, theta), (jj, wavenumber) in itertools.product(
            enumerate(thetas), enumerate(wavenumbers)
        ):
            inner_product_mesh[ii, jj] = np.sum(
                multiplier * sph_harm(theta) * bessel(wavenumber)
            )

        return theta_mesh, wavenumber_mesh, inner_product_mesh

    def inner_product_with_plane_waves_at_infinity(
        self, thetas, wavenumbers, g: Optional[GMesh] = None
    ):
        """
        Return the inner products for each plane wave state in the Cartesian product of thetas and wavenumbers.
    ...
    def get_numeric_eigenstate_basis(
        self, max_energy: float, max_angular_momentum: int
    ) -> Dict[states.QuantumState, states.NumericSphericalHarmonicState]:
        analytic_to_numeric = {}
        ...
        for l in range(max_angular_momentum + 1):
            ...
            number_of_eigenvectors = int(
                wavenumber_max / estimated_spacing
            )  # generate an initial guess based on roughly linear wavenumber steps between eigenvalues

            max_eigenvectors = (
                h.shape[0] - 2
            )  # can't generate more than this many eigenvectors using sparse linear algebra methods
            ...
            while True:
                ...
                if (
                    np.max(eigenvalues) > max_energy
                    or number_of_eigenvectors == max_eigenvectors
                ):
                    break
                ...
                number_of_eigenvectors = int(
                    number_of_eigenvectors
                    * 1.1
                    * np.sqrt(np.abs(max_energy / np.max(eigenvalues)))
                )  # based on approximate sqrt scaling of energy to wavenumber, with safety factor
    ...
    def _apply_length_gauge_transformation(
        self, vector_potential_amplitude: float, g: GMesh
    ) -> GMesh:
        bessel_mesh = special.spherical_jn(
            self.l_mesh,
            self.spec.test_charge * vector_potential_amplitude * self.r_mesh / u.hbar,
        )
        ...
        for l_result in self.l:
            for l_outer in self.l:  # l'
                prefactor = (
                    np.sqrt(4 * u.pi * ((2 * l_outer) + 1))
                    * ((1j) ** (l_outer % 4))
                    * bessel_mesh[l_outer, :]
                )
                for l_inner in self.l:  # l
                    g_transformed[l_result, :] += (
                        g[l_inner, :]
                        * prefactor
                        * core.triple_y_integral(l_outer, 0, l_result, 0, l_inner, 0)
                    )

        return g_transformed

    def gauge_transformation(
        self, *, g: Optional[StateOrGMesh] = None, leaving_gauge: Optional[str] = None
    ) -> GMesh:
        g = self.state_to_g(g)
        if leaving_gauge is None:
            leaving_gauge = self.spec.evolution_gauge
        ...
        if leaving_gauge == core.Gauge.LENGTH:
            return self._apply_length_gauge_transformation(
                -vamp[-1], dipole_to_velocity * g
            )
        elif leaving_gauge == core.Gauge.VELOCITY:
            return dipole_to_velocity * self._apply_length_gauge_transformation(
                vamp[-1], g
            )
