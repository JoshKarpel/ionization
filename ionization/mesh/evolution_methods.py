...
class EvolutionMethod(abc.ABC):
    ...
    def evolve(
        self, mesh: "meshes.QuantumMesh", g: "meshes.GMesh", time_step: complex
    ) -> "meshes.GMesh":
    ...
    @abc.abstractmethod
    def get_evolution_operators(
        self, mesh: "meshes.QuantumMesh", time_step: complex
    ) -> Iterable[mesh_operators.MeshOperator]:
        """
...
class SplitInteractionOperator(EvolutionMethod):
    ...
    def get_evolution_operators(
        self, mesh: "meshes.SphericalHarmonicMesh", time_step: complex
    ) -> "meshes.GMesh":
        tau = time_step / (2 * u.hbar)
        ...
        split_operators = mesh.operators.split_interaction_operators(
            mesh, mesh.operators.interaction_hamiltonian(mesh), tau
        )
