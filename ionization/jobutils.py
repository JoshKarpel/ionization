...
def parse_args(**kwargs):
    ...
    parser.add_argument(
        "--verbosity", "-v", action="count", default=0, help="set verbosity level"
    )
    parser.add_argument(
        "--dry", action="store_true", help="do not attempt to actually submit the job"
    )
...
def get_log_manager(args, **kwargs):
    return si.utils.LogManager(
        "simulacra",
        "ionization",
        "jobutils",
        stdout_level=31 - ((args.verbosity + 1) * 10),
        **kwargs,
    )
...
def check_job_dir(args):
    ...
    if os.path.exists(job_dir_path):
        if not args.overwrite and not clu.ask_for_bool(
            "A job with that name already exists. Overwrite?", default="No"
        ):
...
def ask_mesh_type():
    ...
    logger.warning(
        "Predicted memory usage per Simulation is >{}".format(
            si.utils.bytes_to_str(memory_estimate)
        )
    )
...
def ask_mesh_operators(parameters, *, spec_type):
    ...
    key = clu.ask_for_input(
        ...
        default=core.Gauge.LENGTH.value,
    )
    try:
        method = choices[key]()
    except KeyError:
...
def ask_ide_tunneling(parameters):
    choices = {
        cls.__name__.replace("Rate", ""): cls for cls in tunneling.TUNNELING_MODEL_TYPES
    }
...
def ask_data_storage_tdse(parameters, *, spec_type):
    ...
    if spec_type == mesh.SphericalHarmonicSpecification:
        datastores_questions_defaults += [
            (
                mesh.DirectionalRadialProbabilityCurrent,
...
def create_job_files(
    *, args, specs, do_checkpoints, parameters, pulse_parameters, job_processor_type
):
