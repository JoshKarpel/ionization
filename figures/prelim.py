...
BETTER_GRID_KWARGS = {**si.vis.GRID_KWARGS, "linewidth": 1}
...
QUARTER_PAGE_KWARGS = dict(
    fig_width=13 / 2, fig_height=6.5 / 2, img_format="png", fig_dpi_scale=6
)
...
def title_bg():
    ...
    for which in ("g", "g2"):
        with si.vis.FigureManager(
            f"title_bg__{which}", tight_layout=False, **FULL_PAGE_KWARGS, **PLOT_KWARGS
        ) as figman:
            fig = figman.fig
            ax = fig.add_axes([0, 0, 1, 1])

            getattr(sim.mesh, f"attach_{which}_to_axis")(
                ax, plot_limit=20 * bohr_radius
            )
...
def efield_and_afield():
    ...
    bsw.window = ion.SymmetricExponentialTimeWindow(
        window_time=800 * asec, window_width=30 * asec
    )
    ...
    si.vis.xy_plot(
        ...
        proton_charge
        * bsw.get_vector_potential_amplitude_numeric_cumulative(times)
        / atomic_momentum,
...
def pulse_cep_movie(pulse_type=ion.GaussianPulse, prefix="Gaussian Pulse"):
    ...
    time_bound = 35
    times = np.linspace(
        -time_bound * pw, time_bound * pw, int(2 * time_bound * pw / asec)
    )
    ...
    for plot_bound, postfix in zip(plot_bounds, postfixes):
        ...
        def get_pulse_by_phase(phase):
            pulse = pulse_type(
                ...
                window=ion.SymmetricExponentialTimeWindow(
                    window_time=pw * 8, window_width=0.2 * pw
                ),
            )
        ...
        def efield(times, phase):
            return (
                get_pulse_by_phase(phase).get_electric_field_amplitude(times)
        ...
        def efield_intensity(times, phase):
            return (
                epsilon_0
                * c
                * (np.abs(efield(times, phase) * atomic_electric_field) ** 2)
            )
        ...
        def efield_intensity_envelope(times, phase):
            ...
            return (
                ...
                * c
                * (
                    (pulse.get_electric_field_envelope(times) * pulse.amplitude_time)
                    ** 2
                )
            )
        ...
        def afield(times, phase):
            return (
                get_pulse_by_phase(
                    phase
                ).get_vector_potential_amplitude_numeric_cumulative(times)
                * proton_charge
                / atomic_momentum
            )
        ...
        si.vis.xyt_plot(
            ...
            efield_intensity,
            line_kwargs=[{"linewidth": BIG_LINEWIDTH}],
            y_label=r"Intensity $ P(t) $",
            y_unit="atomic_intensity",
            title=fr"{prefix} w/ $\tau = {uround(pw, asec, 0)} \, \mathrm{{as}}, \, H = {uround(flu, Jcm2)} \, \mathrm{{J/cm^2}} $",
            grid_kwargs=BETTER_GRID_KWARGS,
            **shared_kwargs,
            **BIG_FONTS,
        ...
        si.vis.xyt_plot(
            ...
            line_labels=[r"$ P(t) $", r"$ P(t) $ Envelope"],
            line_kwargs=[
                {"linewidth": BIG_LINEWIDTH},
...
def tunneling_ionization_animation():
    ...
    amplitudes = np.hstack([0, np.geomspace(0.001, 0.3, 599)]) * atomic_electric_field
    ...
    fm = si.vis.xy_plot(
        ...
        elec_pot,
        line_labels=[
            r"$ V_{\mathrm{Coul}} + V_{\mathrm{Field}} $",
            r"$ V_{\mathrm{Coul}} $",
            r"$ V_{\mathrm{Field}} $",
        ],
        line_kwargs=[
            {"animated": True, "linewidth": BIG_LINEWIDTH},
            {"linestyle": "--", "linewidth": BIG_LINEWIDTH},
            {"linestyle": "--", "animated": True, "linewidth": BIG_LINEWIDTH},
        ],
    ...
    si.vis.animate(
        ...
        update,
        amplitudes,
        artists=[fm.elements["lines"][0], fm.elements["lines"][2]],
        length=20,
    )
...
def length_ide_kernel_gaussian():
    ...
    si.vis.xy_plot(
        ...
        line_kwargs=[
            {"color": "black", "linewidth": BIG_LINEWIDTH},
            {"color": "C0", "linewidth": BIG_LINEWIDTH},
            {"color": "C1", "linewidth": BIG_LINEWIDTH},
        ],
        x_label=r"$t-t'$ ($\mathrm{\tau_{\alpha}}$)",
        x_unit=tau,
...
def delta_kick_decomposition_plot():
    ...
    tb = 3
    timess = [
        np.linspace(-tb * pulse_width, tb * pulse_width, 1000),
        np.linspace(-tb * pulse_width, tb * pulse_width, 1000),
    ]
    ...
    for pulse, name, times in zip(pulses, names, timess):
        ...
        si.vis.xxyy_plot(
            ...
            line_kwargs=[
                ...
                {"linestyle": "", "marker": "o"},
                *[
                    {"color": "C1", "linestyle": "-", "linewidth": BIG_LINEWIDTH * 0.75}
                    for _ in kick_times
                ],
            ],
...
def delta_kick_eta_plot():
    etas = np.array([0.2, 0.4, 0.5]) * time_field_unit
    ...
    si.vis.xy_plot(
        ...
        *[curve(periods, eta) for eta in etas],
        ...
        x_label=r"Sine Wave Period Time $t$ ($ \tau_{\alpha} $)",
        x_unit=tau_alpha,
...
def pulse_ffts():
    ...
    pulses = [
        ...
        ion.GaussianPulse.from_omega_min(
            pulse_width=pw,
            fluence=flu,
            phase=phase,
            window=ion.SymmetricExponentialTimeWindow(
    ...
    si.vis.xy_plot(
        ...
        *(epsilon_0 * c * (np.abs(f) ** 2) / len(times) for f in ffts),
        ...
        x_label=r"Frequency $ f $",
        x_unit="THz",
        x_lower_limit=0,
        x_upper_limit=freq_plot_limit,
        ...
        vlines=[2530 * THz],
        vline_kwargs=[
            {"linewidth": BIG_LINEWIDTH, "linestyle": "--", "color": "black"}
        ],
        grid_kwargs=BETTER_GRID_KWARGS,
        **BIG_FONTS,
        **FULL_PAGE_KWARGS,
    ...
    fm = si.vis.xy_plot(
        ...
        *(epsilon_0 * c * (np.abs(f) ** 2) / len(times) / h for f in ffts),
        ...
        x_unit="eV",
        x_lower_limit=0,
        x_upper_limit=freq_plot_limit * h,
    ...
    twin.set_xticks(
        [photon_wavelength_to_energy(wavelength) / eV for wavelength in wavelengths]
    )
    ...
    twin.set_xlabel(
        "Photon Wavelength $\lambda$ ($\mathrm{nm}$)",
        fontsize=BIG_FONTS["font_size_axis_labels"],
    )
    ...
    si.vis.xy_plot(
        ...
        c / freqs,
        *(
            (epsilon_0 * c * (np.abs(f) ** 2) / len(times)) * ((freqs ** 2) / c)
            for f in ffts
        ),
        line_labels=names,
        line_kwargs=[{"linewidth": BIG_LINEWIDTH}, {"linewidth": BIG_LINEWIDTH}],
        ...
        x_unit="um",
        x_lower_limit=c / freq_plot_limit,
        x_upper_limit=1 * um,
...
def multicycle_sine_cosine_comparison(pulse_type, omega_min, postfix):
    ...
    cos_pulse = pulse_type.from_omega_min(
        pulse_width=pulse_width, fluence=fluence, phase=0, omega_min=omega_min
    )
    sin_pulse_aligned = pulse_type.from_omega_min(
        pulse_width=pulse_width, fluence=fluence, phase=pi / 2, omega_min=omega_min
    )

    shift = (pi / 2) / cos_pulse.omega_carrier
    sin_pulse_shifted = pulse_type.from_omega_min(
    ...
    si.vis.xy_plot(
        get_func_name()
        + f"__{pulse_type.__name__}__aligned_envelopes__omega_min={uround(omega_min, twopi * THz)}THz",
        times,
        cos_pulse.get_electric_field_amplitude(times),
        sin_pulse_aligned.get_electric_field_amplitude(times),
        sin_pulse_aligned.get_electric_field_envelope(times)
        * sin_pulse_aligned.amplitude_time,
        ...
        x_label=r"Time $t$",
        x_unit="asec",
        y_label=fr"Electric Field Amplitude ${ion.LATEX_EFIELD}(t)$",
        y_unit="atomic_electric_field",
    ...
    si.vis.xy_plot(
        get_func_name()
        + f"__{pulse_type.__name__}__aligned_zeros__omega_min={uround(omega_min, twopi * THz)}THz",
        times,
        cos_pulse.get_electric_field_amplitude(times),
        sin_pulse_shifted.get_electric_field_amplitude(times),
        cos_pulse.get_electric_field_envelope(times) * cos_pulse.amplitude_time,
        sin_pulse_shifted.get_electric_field_envelope(times)
        * sin_pulse_aligned.amplitude_time,
        line_labels=[r"$\varphi = 0$", r"$\varphi = \pi / 2$"],
        line_kwargs=[
            {"color": "C0", "linewidth": BIG_LINEWIDTH},
...
def hyd__pulse_width_scan__sinc():
    ...
    results_by_phase_and_fluence = {
        (phase, fluence): jp.select_by_kwargs(phase=phase, fluence=fluence)
        for phase in phases
        for fluence in fluences
    }

    metrics = ["final_initial_state_overlap", "final_bound_state_overlap"]
    extra_line_kwargs = dict(linewidth=3)
            ...
            si.vis.xxyy_plot(
                ...
                [
                    *[
                        ...
                        for results in results_by_phase_and_fluence.values()
                    ]
                ],
                [
                    *[
                ...
                title="Pulse Width Scan: Sinc Pulse",
                title_offset=1.075,
...
def ide__pulse_width_scan__sinc():
    jp = clu.JobProcessor.load(
        "job_processors/ide__pw_scan_50-1000as_3flus_3phis__sinc__fast_fixed.job"
    )
    # jp = clu.JobProcessor.load('job_processors/ide__pw_scan__different_energy__fast.job')

    phases = sorted(jp.parameter_set("phase"))
    # fluences = sorted(jp.parameter_set('fluence'))
    fluences = [0.1 * Jcm2, 1 * Jcm2, 10 * Jcm2]
...
def hyd__fluence_scan__sinc():
    jp = clu.JobProcessor.load(
        "job_processors/hyd__flu_scan_v2__5pws_.01-30jcm2_3phis__sinc.job"
    )
    ...
    color_patches = [
        mpatches.Patch(
            color=color,
            label=fr"$ \tau = {uround(pulse_width, asec)} \, \mathrm{{as}} $",
        )
    ...
    results_by_phase_and_pulse_width = {
        (phase, pulse_width): jp.select_by_kwargs(phase=phase, pulse_width=pulse_width)
        for phase in phases
    ...
    for log_x, log_y in itertools.product([True, False], repeat=2):
        ...
        for metric in metrics:
            si.vis.xxyy_plot(
                ...
                [
                    *[
                        [r.fluence for r in results]
                        for results in results_by_phase_and_pulse_width.values()
                    ]
                ],
                [
                    *[
                ...
                line_kwargs=[
                    {
                        "linestyle": phase_to_style[phase],
                        "color": pulse_width_to_color[pulse_width],
                        **extra_line_kwargs,
                    }
                    for phase, pulse_width in results_by_phase_and_pulse_width.keys()
                ],
                title="Fluence Scan: Sinc Pulse",
                title_offset=1.075,
                x_label=r"Fluence $H$",
                x_unit="Jcm2",
                y_label=metric.replace("final_", "").replace("_", " ").title(),
...
def hyd__pulse_width_scan__gaussian():
    jp = clu.JobProcessor.load(
        "job_processors/hyd__pw_scan_v2__50-1000as_3flus_3phis__gaussian__fixed_bounds.job"
    )
    ...
    for log_x, log_y in itertools.product([True, False], repeat=2):
        ...
        for metric in metrics:
            si.vis.xxyy_plot(
                ...
                title="Pulse Width Scan: Gaussian Pulse",
                title_offset=1.075,
                x_label=r"Pulse Width $\tau$",
                x_unit="asec",
                y_label=metric.replace("final_", "").replace("_", " ").title(),
...
def hyd__fluence_scan__gaussian():
    jp = clu.JobProcessor.load(
        "job_processors/hyd__flu_scan_v2__5pws_.01-30jcm2_3phis__gaussian__fixed_bounds.job"
    )
    ...
    metrics = ["final_initial_state_overlap", "final_bound_state_overlap"]
    extra_line_kwargs = dict(linewidth=BIG_LINEWIDTH)
    ...
    for log_x, log_y in itertools.product([True, False], repeat=2):
        ...
        for metric in metrics:
            si.vis.xxyy_plot(
                ...
                title="Fluence Scan: Gaussian Pulse",
                title_offset=1.075,
                x_label=r"Fluence $H$",
                x_unit="Jcm2",
                y_label=metric.replace("final_", "").replace("_", " ").title(),
                ...
                y_log_pad=2,
                x_log_axis=log_x,
                legend_kwargs={"loc": "best", "handles": legend_handles},
                grid_kwargs=BETTER_GRID_KWARGS,
                font_size_axis_labels=35,
                font_size_tick_labels=20,
                font_size_legend=20,
...
def field_properties_vs_phase():
    ...
    for ii, phase in enumerate(tqdm(phases)):
        pulse = ion.SincPulse.from_omega_min(
    ...
    si.vis.xy_plot(
        ...
        phases,
        max_electric_field,
        y_label=rf"$ \max \, \left|{ion.LATEX_EFIELD}_{{\varphi}}(t)\right| $",
        y_unit="atomic_electric_field",
    ...
    si.vis.xy_plot(
        ...
        y_label=rf"$ \max \, e \, \left|{ion.LATEX_AFIELD}_{{\varphi}}(t)\right| $",
...
def instantaneous_tunneling_rate_plot():
    amplitudes = np.linspace(0, 0.1, 1e4) * atomic_electric_field
...
def tunneling_ionization():
    z = np.linspace(-5, 50, 1000) * bohr_radius
    amplitude = 0.02 * atomic_electric_field
...
def richardson_colormap():
    ...
    plot_mesh = 0.25 * r_mesh * np.exp(1j * theta_mesh)

    fig = si.vis.get_figure(fig_width=6, aspect_ratio=1, fig_dpi_scale=6)
    axis = fig.add_axes([0.05, 0.05, 0.9, 0.9], projection="polar")

    color_mesh = axis.pcolormesh(
        theta_mesh, r_mesh, plot_mesh, shading=shading, cmap=colormap, norm=norm
    )
    ...
    axis.grid(
        True, color="black", linewidth=2, alpha=0.4
    ...
    si.vis.save_current_figure(
        get_func_name(), img_format="png", target_dir=OUT_DIR, transparent=False
    )
...
def hyd__cep_scan():
    ...
    for jp_name, pw in zip(jp_names, pws):
        ...
        labels = [r"Initial State Overlap", r"Bound State Overlap"]
        ...
        si.vis.xxyy_plot(
            ...
            line_labels=labels,
            line_kwargs=[
                {"linestyle": metric_to_style[metric], **extra_line_kwargs}
                for metric in metrics
            ],
        ...
        fm = si.vis.xxyy_plot(
            ...
            [
                *[
                    [
                        proton_charge
                        * get_max_or_rms_vector_potential(
                            r.electric_potential, times, which="max"
                        )
                        for r in results
                    ]
                    for metric in metrics
...
def omega_min_scan():
    jp = clu.JobProcessor.load(
        "job_processors/hyd__omega_min_scan__gaussian__fixed_bounds.job"
    )
    ...
    selectors = dict(pulse_width=200 * asec, fluence=1 * Jcm2)
        ...
        [
            *[
                [
                    ...
                    for r in jp.select_by_kwargs(phase=phase, **selectors)
                ]
                for phase in phases
            ]
        ],
...
def ide_symmetry():
    ...
    efields = [
        ion.SincPulse(
            pulse_width=pulse_width * asec, fluence=fluence * Jcm2, phase=phase
        ),
        ion.SincPulse(
            pulse_width=pulse_width * asec, fluence=fluence * Jcm2, phase=pi - phase
        ),
    ]
    ...
    for efield in efields:
        specs.append(
            ide.IntegroDifferentialEquationSpecification(
                efield.phase, electric_potential=efield, **spec_kwargs
            )
        )
    results = si.utils.multi_map(run, specs, processes=2)

    fig = si.vis.get_figure(
        fig_width=si.vis.PPT_WIDESCREEN_WIDTH,
        fig_height=si.vis.PPT_WIDESCREEN_HEIGHT,
        fig_dpi_scale=6,
    )
    ...
    for result, cep, color, style in zip(
        results,
        (r"$\varphi = \pi / 4$", r"$\varphi = 3 \pi / 4$"),
        ("C0", "C1"),
        ("-", "-"),
    ):
        ax_lower.plot(
            result.times / asec,
            result.spec.electric_potential.get_electric_field_amplitude(result.times)
            / atomic_electric_field,
            color=color,
            linewidth=BIG_LINEWIDTH,
            label=cep,
    ...
    efield_1 = results[0].spec.electric_potential.get_electric_field_amplitude(
        results[0].times
    )
    ...
    title = ax_upper.set_title(
        r"IDE CEP Comparison: $\tau = 200 \, \mathrm{as}, \; H = .3 \, \mathrm{J/cm^2}$",
        fontsize=35,
    )
    title.set_y(1.15)
    ...
    si.vis.xxyy_plot(
        ...
        [
            *[
                r.spec.electric_potential.get_electric_field_amplitude(r.times)
                for r in results
            ]
        ],
        line_labels=[r"$\varphi = \pi / 4$", r"$\varphi = 3 \pi / 4$"],
        line_kwargs=[
            {"linewidth": BIG_LINEWIDTH, "color": si.vis.GREEN},
...
def delta_kick_cosine_sine_comparison():
    ...
    cos_eta, sin_eta = get_cosine_and_sine_etas(
        pulse_width=200 * asec, fluence=0.05 * Jcm2
    )
    ...
    si.vis.xy_plot(
        ...
        * double_kick__interference(sin_beta, delta_t, omega_alpha, tau_alpha),
        ...
        vlines=[tau_alpha],
        vline_kwargs=[
            {"linestyle": "--", "color": "black", "linewidth": BIG_LINEWIDTH}
        ],
        legend_kwargs={
            "loc": "upper right",
...
def pulse_prefactors_vs_properties():
    ...
    fluences_sparse = np.array([0.1, 1, 5, 10, 20]) * Jcm2
    ...
    fluences_log = np.geomspace(0.01, 20, 1e3) * Jcm2
    si.vis.xy_plot(
...
if __name__ == "__main__":
    with logman as logger:
        ...
        movies = [
            functools.partial(
                pulse_cep_movie, pulse_type=ion.GaussianPulse, prefix="Gaussian Pulse"
            ),
            functools.partial(
                pulse_cep_movie, pulse_type=ion.SincPulse, prefix="Sinc Pulse"
            ),
            tunneling_ionization_animation,
        ]

        long_computation = [ide__cep_scan]
