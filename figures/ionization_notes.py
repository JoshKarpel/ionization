...
PLOT_KWARGS_LIST = [
    dict(
        fig_width=si.vis.points_to_inches(498.66), target_dir=OUT_DIR, img_format="pdf"
    ),
    dict(
        fig_width=si.vis.points_to_inches(498.66), target_dir=OUT_DIR, img_format="pgf"
    ),
    dict(
...
def sinc_pulse_power_spectrum_full():
    ...
    power = np.where(np.abs(omega) < upper, 1, 0) * np.where(
        np.abs(omega) > lower, 1, 0
    )

    ax.fill_between(omega, 0, power, alpha=1, edgecolor="black", facecolor="darkgray")

    delta_line_y_coord = 0.75
    plt.annotate(
        s="",
        xy=(lower, delta_line_y_coord),
        xytext=(upper, delta_line_y_coord),
        textcoords="data",
        arrowprops=dict(arrowstyle="<->"),
    )
    plt.text(c + 0.1 * (upper - c), delta_line_y_coord + 0.025, r"$\Delta_{\omega}$")
    ...
    plt.text(-c - 0.1 * (lower - c), delta_line_y_coord + 0.025, r"$\Delta_{\omega}$")
...
def sinc_pulse_power_spectrum_half():
    ...
    plt.text(
        carrier + 0.1 * (upper - carrier),
        delta_line_y_coord + 0.025,
        r"$\Delta_{\omega}$",
    )
...
def gaussian_pulse_power_spectrum_half():
    ...
    omega = np.linspace(0, 1, 1000)
    power = np.exp(-0.5 * (((omega - carrier) / sigma) ** 2)) / np.sqrt(twopi) / sigma
    max_power = np.max(power)
    power /= max_power

    ax.fill_between(omega, 0, power, alpha=1, edgecolor="black", facecolor="darkgray")

    delta_line_y_coord = 0.3
    plt.annotate(
        s="",
        xy=(carrier - delta / 2, delta_line_y_coord),
        xytext=(carrier + delta / 2, delta_line_y_coord),
        textcoords="data",
        arrowprops=dict(arrowstyle="<->"),
    )
    plt.text(carrier + sigma / 5, delta_line_y_coord - 0.1, r"$\Delta_{\omega}$")
...
def finite_square_well():
    ...
    ax.set_xticklabels([r"$0$", r"$  -\frac{a}{2} $", r"$  \frac{a}{2} $"])
...
def finite_square_well_energies():
    ...
    z_0 = 6 * pi / 2 + 0.5 * np.sqrt(
        1
    )  # must make it numpy data type so that the optimizer doesn't panic
    ...
    ax.plot(
        ...
        sqrt,
        color="black",
        label=r"$   \sqrt{  \left( \frac{z_0}{z} \right)^2  -1 }  $",
    )
    ...
    ax.set_xticklabels(
        [r"$z_0$", r"$0$", r"$\frac{\pi}{2}$"]
        + [r"${} \frac{{\pi}}{{2}}$".format(n) for n in range(2, int(z_0 + 5))]
    )
    ...
    for n in np.arange(1, z_0 / (pi / 2) + 1.1, 1):
        ...
        if n % 2 != 0:  # n is odd
            intersection = optimize.brentq(
                lambda x: np.tan(x) - np.sqrt(((z_0 / x) ** 2) - 1),
                left_bound,
                right_bound,
            )
        else:  # n is even
            intersection = optimize.brentq(
                lambda x: (1 / np.tan(x)) + np.sqrt(((z_0 / x) ** 2) - 1),
                left_bound,
                right_bound,
            )
...
def a_alpha_v2_kernel_gaussian():
    ...
    ax.plot(
        ...
        np.real(y),
        color="C0",
        label=r"$  \mathrm{Re} \left\lbrace K(t-t') \right\rbrace  $",
    )
    ax.plot(
        dt,
        np.imag(y),
        color="C1",
        label=r"$  \mathrm{Im} \left\lbrace K(t-t') \right\rbrace   $",
    )

    ax.set_xlabel(r"$   t-t'  $")
    ax.set_ylabel(
        r"$   K(t-t') = \left(1 + i \frac{t-t'}{\tau_{\alpha}}\right)^{-3/2}  $"
    )
    ...
    ax.set_yticks([0, 1, -1, 0.5, -0.5, 1 / np.sqrt(2)])
    ax.set_yticklabels([r"$0$", r"$1$", r"$-1$", r"$1/2$", r"$-1/2$", r"$1/\sqrt{2}$"])
...
def ide_solution_sinc_pulse_cep_symmetry(phase=0):
    ...
    grid_spec = matplotlib.gridspec.GridSpec(
        2, 1, height_ratios=[2.5, 1], hspace=0.07
    )  # TODO: switch to fixed axis construction
    ...
    efields = [
        ion.SincPulse(
            pulse_width=pulse_width * asec, fluence=fluence * Jcm2, phase=phase * pi
        ),
        ion.SincPulse(
            pulse_width=pulse_width * asec, fluence=fluence * Jcm2, phase=-phase * pi
        ),
    ...
    spec_kwargs = dict(
        time_initial=-20 * pulse_width * asec,
        time_final=20 * pulse_width * asec,
        time_step=1 * asec,
        error_on="da/dt",
        eps=1e-3,
        minimum_time_step=0.1 * asec,
    ...
    for result, cep, color, style in zip(
        results,
        (r"$\mathrm{CEP} = \varphi$", r"$\mathrm{CEP} = -\varphi$"),
        ("C0", "C1"),
        ("-", "-"),
    ):
        ax_lower.plot(
    ...
    ax_lower.yaxis.set_label_coords(-0.125, 0.5)
    ...
    ax_lower.set_yticks(
        [0, field_max, field_max / np.sqrt(2), -field_max, -field_max / np.sqrt(2)]
    )
    ...
    ax_lower.set_ylim(
        field_min - (0.125 * field_range), field_max + (0.125 * field_range)
    )
...
def photons_at_a_glance():
    ...
    for kwargs in PLOT_KWARGS_LIST:
        for (
            (x_data, x_name, x_label, x_unit),
            (y_data, y_name, y_label, y_unit),
        ) in itertools.combinations(zip(data, names, axis_labels, units), 2):
            print(x_name, y_name)
            si.vis.xy_plot(
...
def sine_squared_pulse(phase=0, number_of_cycles=1):
    ...
    t_bound = 0.5 * number_of_cycles * pulse.period_carrier
